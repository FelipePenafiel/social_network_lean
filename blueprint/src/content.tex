...
\begin{lemma}[The bound $\eta$, Remark~5]
  \label{lem:eta}
  \lean{SocialNetwork.eta, SocialNetwork.eta_le_pathMeasure_positivePressure}
  \leanok
...
\begin{proof}
  ...
  rate at most $1$, and there are at most $MN$ of them.  \textbf{Not
  formalised}: what is missing is the comparison showing the worst case over
  $\steep$ is attained on $\ladder$.
\end{proof}
...
\begin{remark}[The smallest maximum of a non-null row, Remark~8]
  \label{rem:remark8}
  \lean{SocialNetwork.Bias.le_max_pressure, SocialNetwork.Bias.exists_count_ge}
  \leanok
  \uses{def:biased-state}
...
\begin{proof}
  ...
  $\max_p u(a,p) \ge k(1+\gamma) - \gamma n_a = k(M-1)\alpha + \gamma(M-r)
   \ge (M-1)\alpha$.  \textbf{Partly formalised}: the pigeonhole step is
  \texttt{SocialNetwork.Bias.exists\_count\_ge}; the arithmetic is not.
\end{proof}
...
Remark~\ref{rem:green}: does it follow what the paper wrote?  Thirteen proofs
carry \texttt{\textbackslash leanok}.

\medskip
\noindent\textbf{Follow the paper's proof} (four).
Propositions~\ref{prop:low-pressure} and~\ref{prop:confinement}, the gap
estimate of Lemma~\ref{lem:gap}, and the bound of
Proposition~\ref{prop:greedy-prob}.  These are the only statements in the paper
whose proofs are both given in full there and formalised here, and each Lean
proof reproduces the written argument.  Two places carry a note: the passage
...
and the iteration in Proposition~\ref{prop:greedy-prob} goes through the
Ionescu--Tulcea kernels rather than the conditioning of~(10), Mathlib offering
nothing of that shape.

\medskip
\noindent\textbf{Supply a step the paper asserts} (five).
Lemmas~\ref{lem:state-stable}, \ref{lem:consensus-pos}, \ref{lem:steep-stable},
\ref{lem:favouring-max} and~\ref{lem:closing}.  Each corresponds to a sentence
in the paper of the form ``note that'', ``indeed'', or an unremarked clause
inside a proof.  All five are true and none is hard; what they are not is
checks of anything the paper argued.

\medskip
\noindent\textbf{No counterpart in the paper} (four).
Lemmas~\ref{lem:trust}, \ref{lem:ladder-consensus}, \ref{lem:state-along}
and~\ref{lem:consensus-ladder}, together with the measurability lemmas and the
...
\begin{remark}[``By definition'' in Proposition~\ref{prop:reach-ladder}]
...
Four statements were listed here as neither deep nor blocked.  They are now
proved, and the paragraph is kept as the record of what they cost.  All four
fall in the third case of Remark~\ref{rem:green}: the paper proves none of
them, so none of these is a check of anything it wrote.
...
Still here, and still routine: Lemma~\ref{lem:eta} and
Remark~\ref{rem:remark8}.
...
\begin{enumerate}
  ...
  \item Lemma~\ref{lem:eta}, the bound $\eta$ of Remark~5.  The arithmetic is
    available; what is missing is the comparison showing that the worst case
    over $\steep$ is attained on $\ladder$.  This is the first genuinely
    mathematical item that is blocked neither on Mathlib nor on the paper.
  \item Remark~\ref{rem:remark8}, whose pigeonhole step is already done.
