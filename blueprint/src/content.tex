...
\begin{proof}
  ...
  (M+1)MN^2 e^{-\beta/(M-1)}$ by \ref{rem4}. \textbf{Not formalised}.
  ...
  which is to say behind \ref{lem19} and~\ref{lem20}.
\end{proof}
...
\ref{thm2-2}, \ref{cor11}, \ref{aux-hitting-rate}, \ref{eq19} and~\ref{rem6}
join this queue rather than the one above once their proofs are written: none
of them is blocked on anything today, and each rests on \ref{prop7} when it is
written, hence on \ref{lem19} and~\ref{lem20}.  They are listed as not done
rather than as blocked because that is what they are: the obstruction is
...
and the last of those groups now has \ref{thm2-2}, \ref{cor11},
\ref{aux-hitting-rate} and~\ref{eq19} in it beside \ref{rem6}.
...
\begin{enumerate}
  \item \textbf{The four continuous-time estimates around part~2 of Theorem~2}:
    \ref{aux-hitting-rate} first, since \ref{thm2-2} is its limit, \ref{cor11}
    and \ref{eq19} after it, and \ref{rem6} stated at last.  This is what is
    left that is nobody's fault, now that \ref{aux-measurable-hitting},
    \ref{prop18}, \ref{thm4} part~1, \ref{prop9}, \ref{cor10}
    and~\ref{aux-greedy-avoids} are closed.  Nothing outside this repository is
    needed for any of them: the process, its holding times and its hitting
    times are all built, and what is missing is an exponential race and a union
    bound.  None of them will be \emph{proved} before item~2 --- they rest on
    \ref{prop7} --- but each turns green with no edit the moment it is.
