...
\begin{remark}[How the process is built in Lean]
  ...
  obligation this construction leaves open is
  Lemma~\ref{lem:measurable-hitting}, which is not of the same kind and has a
  node of its own.
\end{remark}
...
\end{lemma}
\begin{proof}
  \textbf{Not formalised, and no counterpart in the paper}, which does not
  address measurability anywhere.  It is separated from
  Remark~\ref{rem:construction} because it is the one statement there that is
  not a cylinder argument: the hitting time is an infimum over the
  \emph{uncountable} family $\{t \ge 0\}$, and reducing it to a countable one
  needs right-continuity of $t \mapsto \Uproc{u}_t(\omega)$, which holds only
  almost surely.  The obstruction is written out at the end of
...
compose, the statement is a citation rather than a result of this paper, or
the Lean statement itself has still to be settled.  \texttt{STATUS.md} at the
root of the repository is the generated index of all four, statement by
statement; the paragraphs below are the argument behind it.
...
\subsection*{The statement itself is not settled}

Lemma~\ref{lem:measurable-hitting}, alone.
...
promised here.  The biased twin
\texttt{SocialNetwork.Bias.measurable\_biasedHittingTimeCts} is the same
statement over $\Salpha$ and is open for the same reason; the two share
Lemma~\ref{lem:measurable-hitting}.

This is the only unproved statement in the repository that is nobody's fault:
not a gap in Mathlib, not a gap in the paper, not a citation.  What it needs is
a decision about what the statement should say, and that decision is about what
the paper means by the hitting time, so it belongs to the authors.
...
Remark~\ref{rem:green}: does it follow what the paper wrote?  Thirty-four
...
of its own, so it counts thirty-five entries.

\medskip
\noindent\textbf{Follow the paper's proof} (sixteen).
Propositions~\ref{prop:low-pressure}, \ref{prop:confinement},
\ref{prop:reach-ladder}, \ref{prop:biased-low-pressure},
\ref{prop:biased-bounded}
and~\ref{prop:biased-greedy-prob}, the gap estimate of Lemma~\ref{lem:gap},
...
\medskip
\noindent\textbf{No counterpart in the paper} (nine).
