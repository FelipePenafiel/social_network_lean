...
\end{theorem}
\begin{proof}
  \uses{aux-doeblin, prop6, prop8}
  This is the Doeblin step, and it is the keystone: \ref{thm1-2},
  \ref{thm2-1}, \ref{thm3}, \ref{thm25}, \ref{thm27} and \ref{thm31} all rest
  on it.  \textbf{Not formalised}, and what is missing is now precise rather
  than wholesale.

  \emph{The criterion} is \ref{aux-doeblin}, and it is proved.  What it wants is
  a minorisation, at one state, of some iterate of the skeleton kernel, uniform
  over $\Sspace$.

  \emph{The minorisation} is the argument at p.~17 of the paper, and it is
  work in this repository rather than a gap in Mathlib.  Its three ingredients
  are: that $N$ expressions of $o$, made by the actors in decreasing order, send
  \emph{any} matrix to the canonical ladder $l^o$ of \ref{def1} --- each row is
  reset when its actor expresses and then rises by $M-1$ once per later
  expression --- which is the paper's~(12); that after $N$ greedy expressions
  the matrix is entrywise between $-MN$ and $N$, which is \ref{prop6}, with
  probability at least $\zeta_\beta^N$ by \ref{prop8}; and that from a matrix so
  bounded the $N$ expressions above each have probability bounded below, the
  rates of \ref{def3} being ratios of exponentials of bounded exponents.
  ...
  what \texttt{blueprint/blueprint.md} records as absent.
\end{proof}
...
\ref{aux-doeblin} has changed what the first item below means.  Doeblin's
\emph{criterion} is no longer missing: it is proved here, for a general Markov
kernel on a countable space, and it is the half that gives uniqueness.  What is
missing at \ref{thm1-2-skeleton} is the \emph{existence} half --- the
construction of an invariant measure from one excursion of a positive recurrent
chain --- and, separately, the minorisation this chain satisfies, which is work
in this repository and not a gap in Mathlib.  Its node says which three pieces.
...
\ref{note-green}: does it follow what the paper wrote?  Fifty proofs
...
own, so it counts fifty-one entries.  Every one of them is named, and
...
\medskip \noindent\textbf{Follow the paper's proof} (twenty-five). \ref{prop5},
...
\medskip \noindent\textbf{No counterpart in the paper} (thirteen).
...
and~\ref{aux-doeblin}, together with the plumbing of the sample space, which is
not made of numbered statements and so appears here only inside definitions and
remarks.

The last three are the substantial ones, and none of them checks anything the
