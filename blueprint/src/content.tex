...
\begin{remark}[What a green node claims, and what it does not]
  \label{rem:green}
  A green node means the Lean proof is complete.  On its own it says nothing
  about whether the \emph{paper's} proof has been checked.  Three different
  things can stand behind one:

  \begin{itemize}
    \item the paper's argument, formalised --- the usual case;
    \item a different route to the same statement, which confirms the statement
      and leaves the paper's argument unexamined;
    \item a statement the paper gives no proof of at all --- measurability, the
      plumbing of the sample space, a step waved through as obvious.
  \end{itemize}

  The distinction is not pedantry: examining the paper's arguments is how
  Lemmas~\ref{lem:lemma19} and~\ref{lem:lemma20} came to be flagged, and a
  silent detour around one is a check that was never performed.  So the proof
  environments below describe the argument that is \emph{in Lean}, and where
  that is not the paper's argument, or where the paper has none, they say so.
\end{remark}
