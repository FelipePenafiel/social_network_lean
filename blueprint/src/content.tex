...
\begin{remark}[How the process is built in Lean]
  ...
  \lean{SocialNetwork.ctsPathMeasure, SocialNetwork.jumpTime, SocialNetwork.explosionTime, SocialNetwork.process, SocialNetwork.transitionKernel, SocialNetwork.hittingTimeCts, SocialNetwork.measurable_process, SocialNetwork.measurable_hittingTimeCts}
  \leanok
  \uses{def:skeleton}
...
\begin{definition}[The biased state space, eq.~(6)]
  ...
  \lean{SocialNetwork.Bias.Profile, SocialNetwork.Bias.IsBiasedState, SocialNetwork.Bias.Memory, SocialNetwork.Bias.Memory.pressure, SocialNetwork.Bias.Memory.heard, SocialNetwork.Bias.IsBiasedState.two_mul_totalHeard_ge, SocialNetwork.Bias.sum_ge_of_injective}
  \leanok
...
\begin{definition}[The near-greedy event, Remark~7]
  ...
  \lean{SocialNetwork.Bias.IsNearGreedyAt, SocialNetwork.Bias.nearGreedyEvents, SocialNetwork.Bias.IsBiasedGreedyAt, SocialNetwork.Bias.biasedGreedyEvents, SocialNetwork.Bias.measurableSet_nearGreedyEvents, SocialNetwork.Bias.measurableSet_biasedGreedyEvents}
  \leanok
  \uses{def:biased-state}
  \[
...
\subsection*{Routine, not done}

Neither deep nor blocked: \texttt{SocialNetwork.measurable\_process} and
\texttt{measurable\_hittingTimeCts} (\texttt{jumpCount} is a countable Boolean
combination of the events $\{T_n \le t\}$), the two measurability lemmas for the
biased greedy events, Lemma~\ref{lem:eta}, and Remark~\ref{rem:remark8}.

One of these is a \textbf{Mathlib gap in its own right}:
\texttt{SocialNetwork.Bias.sum\_ge\_of\_injective}, that $N$ distinct naturals
sum to at least $0 + 1 + \dots + (N-1)$.  Mathlib has no lemma to this effect
and none from which it follows in one step; the natural route is
\texttt{Finset.orderEmbOfFin} plus ``a strictly monotone $\mathrm{Fin}\ N \to
\mathbb{N}$ dominates the identity'', but \texttt{StrictMono.le\_apply} is
stated only for endomorphisms.
...
\begin{enumerate}
  \item The routine items above: the two measurability lemmas for
    \texttt{process} and \texttt{hittingTimeCts}, the two for the biased greedy
    events, and \texttt{sum\_ge\_of\_injective}.  None needs new theory, and
    together they remove the only unproved statements here that are nobody's
    fault.
