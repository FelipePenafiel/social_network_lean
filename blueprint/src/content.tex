...
\begin{proof}
  ...
  \ref{aux-hitting-rate} was in the way.
\end{proof}
...
\begin{proof}
  ...
  that is not $0$, to which \ref{thm2-2} applies. \textbf{Not formalised}, and
  --- since \ref{thm2-2} does not --- no longer waiting on Doeblin.
  ...
  the paper}; \texttt{FOR-THE-AUTHORS.md} §2.11.

  What it still waits on is a restart of the continuous-time process at $T_1$,
  which this library does not have: \texttt{Trajectory.shift} restarts the
  skeleton, and nothing yet restarts $\Uproc{u}$.
\end{proof}
...
\begin{lemma}[The clock of a greedy run]
  ...
  \lean{SocialNetwork.exp_le_totalRate, SocialNetwork.state_ne_zero_of_greedy, SocialNetwork.zeta_pow_le_ctsPathMeasure_greedyEvents, SocialNetwork.ctsPathMeasure_greedy_holdingTime_gt, SocialNetwork.ctsPathMeasure_holdingTime_nonpos, SocialNetwork.hittingTimeCts_le_jumpTime, SocialNetwork.expMeasure_Ioi_of_nonneg}
  \leanok
...
\begin{proof}
  ...
  there to \ref{cor11}, which is itself stated as a limit.  What it actually
  decomposes is \ref{cor11}'s own proof --- the waiting time at $0$, then
  \ref{thm2-2} --- which is how Corollary~13 of [GL24] reads at $M = 2$.  Note
  that $\tau$ is
...
\ref{aux-hitting-rate} and~\ref{thm2-2} joined this queue as soon as their
proofs were written, and \ref{cor11}, \ref{eq19} and~\ref{rem6} will: none of
them is blocked on anything today, and each rests on \ref{prop7} once written,
hence on \ref{lem19} and~\ref{lem20}.  They are listed as not done rather than
as blocked because that is what they are: the obstruction is recorded once, at
\ref{lem19} and~\ref{lem20}, and not copied down the graph.
...
and the last of those groups now has \ref{cor11} and~\ref{eq19} in it beside
\ref{rem6}.
...
library, so they moved to the last group, work not done --- and the first of
them is now \emph{proved}, from \ref{aux-holding-times} and \ref{prop7}, which
is the best evidence that the reclassification was right.  What survives as a
...
\begin{enumerate}
  \item \textbf{What is left around part~2 of Theorem~2}.  \ref{thm2-2} and
    \ref{aux-hitting-rate} are written, and with them \ref{aux-holding-times},
    which is proved outright.  \ref{cor11} and \ref{eq19} are what remains, and
    they want one lemma between them: a restart of $\Uproc{u}$ at the first
    jump.  \texttt{Trajectory.shift} restarts the skeleton and nothing yet
    restarts the continuous-time process; the pieces are there, since
    \ref{aux-holding-times} already reads the law of one step off the kernels.
    \ref{rem6} can be stated at any time.
    ...
    and~\ref{thm2-2} are closed.  Nothing outside this repository is needed:
    the process, its holding times and its hitting times are all built.
    Neither will be \emph{proved} before item~2, since both rest on
    \ref{prop7}; but a written proof turns green with no edit the moment its
    ancestors do, which is why writing them is worth doing first.
