...
\begin{proof}
  ...
  processes.  \textbf{Not formalised}, and \ref{eq13} is now the whole of what
  is missing: \ref{thm1-2-skeleton} is proved, so $\muskel$ exists and is
  unique, and the transfer is the only step between it and $\muinv$.
\end{proof}
...
\begin{proof}
  ...
  This is the Doeblin step, and it was the keystone: \ref{thm1-2},
  \ref{thm2-1}, \ref{thm3}, \ref{thm25}, \ref{thm27} and \ref{thm31} all rest
  on it.  \textbf{It is now proved, both halves.}
  ...
  Two things are worth recording about what this did \emph{not} need.  It needs
  no Markov-chain theory beyond \texttt{Kernel.Invariant} --- the existence
  half is an excursion measure, which on a countable state space is a sum in
  $[0,\infty]$ rather than a limit --- so the entry that stood here, ``the
  classical construction of an invariant measure from one excursion is exactly
  the theory Mathlib does not have'', was wrong about what it would cost.  And
  it does not need the paper's $N \ge 3$: the descending sweep reaches $l^o$
  for every $N \ge 1$, so the Lean statement drops that hypothesis.
\end{proof}
...
\begin{proof}
  ...
  the paper}; \texttt{FOR-THE-AUTHORS.md} §2.11.  It is under this reading, and
  only under it, that the corollary is now proved.
\end{proof}
...
\begin{proof}
  ...
  \ref{lem19} and~\ref{lem20}, whose work is to \emph{get} to $C^o$.  The edge
  to \ref{thm2-2} was a second neighbour mistaken for a dependency, after the
  edge to \ref{cor11} this node used to carry.
...
\begin{proof}
  ...
  Lean statement covers the second only, and the statement above has been
  narrowed to match it; it used to claim both.
  ...
  \textbf{Two corrections to record}, because this blueprint has said otherwise
  twice.  It first said the biased minorisation was routine; then that the box
  it needs is \ref{prop22} and that only a constant weaker than the paper's
  would avoid it; then that \ref{prop17} gives the box and \ref{prop22} is not
  needed, and on that basis the node was marked proved.  The first two were
  wrong on the mathematics.  The third was right on the mathematics and wrong on
  what this repository is for: \ref{prop17} does give a box, by an argument
  Appendix~C does not make, and proving a statement of the paper by an argument
  the authors have not written is precisely what \ref{note-green} forbids.  The
  node is back on Appendix~C's route, with the obstruction it carries.
\end{proof}
...
The fourth group had been emptied; \ref{thm1-2-skeleton} has re-opened it, and
that is progress rather than regress.  Three statements were listed as blocked
on Mathlib only because they waited on $\muskel$ to exist.  It does now, so what
they wait on is work in this repository and nothing else, and they have moved
into the fourth group.  The wall that cannot be climbed from here is smaller by
the same three.

\subsection*{Blocked on Mathlib}

\textbf{The keystone has left this list.}  \ref{thm1-2-skeleton} is proved, and
both of the things it needed are supplied here: \ref{aux-minorisation}, the
minorisation of p.~17, which was always work in this repository rather than a
gap in Mathlib, and \ref{aux-doeblin}, Doeblin's criterion, which is.  The
entry that stood here was wrong about the second half.  It read that ``the
classical construction of an invariant measure from one excursion is exactly
the theory Mathlib does not have'' --- the premise is right and the conclusion
was not, because on a \emph{countable} state space that construction needs no
theory at all: the excursion measure is a sum in $[0,\infty]$, the minorisation
makes it summable, and invariance is the last-exit decomposition written in
coordinates.  Items~1 and~2 below are still absent from Mathlib; they no longer
block anything here.

\ref{thm1-1}, \ref{thm1-2}, \ref{thm2-1}, \ref{thm3}, \ref{thm4} and their
auxiliaries.  Verified against the pinned revision
...
\begin{itemize}
  \item \textbf{Doeblin's condition $\Rightarrow$ a unique invariant measure}:
    ...
    lemma.  (This item no longer blocks anything here: \ref{aux-doeblin} proves
    the implication --- existence \emph{and} uniqueness --- and it is the
    clearest thing in this repository to offer upstream.)
  \item \textbf{Recurrence for chains, and return times}: nothing.  Poincaré
    ...
    Nor was positive recurrence needed for \ref{thm1-2-skeleton}: the
    minorisation gives the geometric tail directly, and the excursion measure
    is built from it without any recurrence theorem.)
...
\ref{thm2-1} and~\ref{thm3} live, in the paper's own architecture, on the
...
processes.  With $\muskel$ now in hand, none of the discrete-time items blocks
anything: \ref{thm2-1} waits on \ref{prop9}, which waits on \ref{prop7} and so
on \ref{lem19} and~\ref{lem20}, and those are blocked on the paper rather than
on Mathlib.  What is left on this list in discrete time is therefore
\ref{eq13}, whose statement the authors may want to change, and nothing else.

What is \emph{not} on this list, and was until \ref{thm2-1} and~\ref{thm2-2}
were split apart, is part~2 of Theorem~2 --- together with \ref{cor11}, which
stood here as ``Doeblin, through Theorem~2''.  A statement blocked on Doeblin is
one that needs $\muinv$ to exist before it can be proved; part~2 never mentions
$\muinv$ at all.  See the node of \ref{thm2-2}.

\subsection*{Blocked on the paper}

\ref{lem19}, \ref{lem20} and~\ref{aux-greedy-avoids}, and \ref{prop22}; through
them \ref{prop7}, \ref{prop9}, \ref{prop23} and~\ref{thm25}.  See
\ref{note-lem19}, \ref{note-lem20} and~\ref{note-prop22}, and the node of
...
rather than guessed at in Lean.  The proofs downstream are written out in full
all the same, so that each inherits \texttt{sorryAx} from the obstruction and
from nothing else: \ref{prop7} and \ref{prop9} from \ref{lem19} and~\ref{lem20},
\ref{thm25} from \ref{prop22}.  \ref{prop7}'s assembly takes a route that does
not need the step the written proof asserts --- the one departure of that kind
in this repository, and the reason is written at its node.

\ref{aux-hitting-rate}, \ref{thm2-2} and~\ref{cor11} joined this queue as soon
as their proofs were written.  \ref{eq19} and~\ref{rem6} were expected to and did
not: written out, neither proof passes through \ref{thm2-2}, and both are
proved.  The obstruction is recorded once, at \ref{lem19} and~\ref{lem20}, and
not copied down the graph --- which is the only reason the mistake was cheap:
nothing else had been filed behind it.

\subsection*{Cited from outside the paper}

Two statements are not results of arXiv:2607.19651 at all, and nothing in this
...
\ref{aux-hitting-rate} and~\ref{eq19} stood here as two more, and they do not
belong: see below.

\subsection*{Work, not blocked}

\ref{eq13} and \ref{thm1-2}.  Nothing stands in the way of either; neither is
done.  \ref{thm25} was listed here, then marked proved, and is now in the group
above: its minorisation is written out on Appendix~C's own ingredients and rests
on \ref{prop22}.  The proof that avoided \ref{prop22} is recorded at
\ref{aux-biased-minorisation}, and closing the node is a decision for the
authors rather than work waiting here.

\ref{eq13} is the stationary-law transfer, and it is the only one with a
question attached: this repository has had its statement filed as one the
authors may want to revisit.  Its Lean statement takes both measures as
hypotheses, so it never needed $\muskel$ to exist in the first place; what it
needs is the work, and a look from the authors first.
\ref{thm1-2} is then immediate from it and \ref{thm1-2-skeleton}.
...
Mathlib, blocked on the paper, cited from outside it, or simply not done ---
and the last of those groups is now \emph{empty}: \ref{cor11}, \ref{eq19}
and~\ref{rem6} have all been written, and every numbered statement of the paper
is stated in Lean.
...
\textbf{The two displays inside proofs}, \ref{aux-hitting-rate}
and~\ref{eq19}, stood in the subsection above as citations, on the ground that
the numbered statements they are attributed to --- part~2 of Theorem~2 and
\ref{cor11} --- are limits, and a limit has thrown the rate away.  The second
half of that is right and is why they are stated here at all: \ref{lem13} uses
the inequalities and cannot use the limits.  The first half is wrong.  They are
not attributed to anything outside the paper; they are steps of the paper's
\emph{own} proofs, left as displays instead of being numbered, and [GL24]
writes both of them out --- the first as its equations (16)--(18), the second
as its Corollary~13, both at p.~19.  Nothing in them is out of reach of this
library, so they moved to the last group, work not done --- and both are now
written, the first from \ref{aux-holding-times} and \ref{prop7} and the second
from \ref{aux-restart}, which is the best evidence that the reclassification
was right.  \ref{eq19} is in fact \emph{proved}, sorry-free: nothing it uses is
blocked on anything.  So is \ref{rem6}, for the same kind of reason --- see its
node.  What survives as a
question for the authors is only whether either should be numbered, and that
question is asked in \texttt{FOR-THE-AUTHORS.md} rather than recorded as an
obstruction here.  The lesson is again the one above: a citation is not an
obstruction until one has checked what is actually being cited.
...
\texttt{scripts/status.py} now checks that: a proof that carries
\texttt{\textbackslash leanok} and appears in none of the three groups below is
an unaudited proof, and it fails the build.  The check was added because the
list had drifted --- nine proofs were written without a line here before anyone
noticed.
...
paper.  This repository was briefly built that way, with \ref{thm25} marked
proved; that was the one place where a statement of the paper had been
established by an argument the authors have not written, and it has been undone.
Both halves of the observation are recorded, here and in
\texttt{FOR-THE-AUTHORS.md}, as a decision for the authors: weakening
\ref{prop17} to $\gamma > 0$ is free and makes \ref{prop22} unnecessary for
Theorem~25.
...
\begin{enumerate}
  \item \textbf{Nothing is left that is nobody's fault.}  This item held
    \ref{aux-measurable-hitting}, then \ref{prop18}, \ref{thm4} part~1,
    \ref{prop9}, \ref{cor10}, \ref{aux-greedy-avoids}, \ref{aux-hitting-rate},
    \ref{thm2-2}, \ref{aux-restart}, \ref{cor11}, \ref{eq19} and last
    \ref{rem6}; all of them are closed, and every numbered statement of the
    paper is stated in Lean.  What remains unproved is blocked on Mathlib,
    blocked on the paper, or cited from outside it, and the next two items are
    those.
  \item \ref{lem19} and~\ref{lem20}, with the repairs of \ref{note-lem19}
    and~\ref{note-lem20}.  Worth settling with the authors first, since the
    repairs change the written proofs. \ref{prop7} then follows by assembling
    them with \ref{aux-consensus-ladder}, and \ref{prop9}, \ref{cor10}
    and~\ref{aux-greedy-avoids} --- all three of them already written --- are
    proved outright.
  \item \textbf{Doeblin's condition $\Rightarrow$ a unique invariant measure for
    ...
    skeleton half \ref{thm1-2-skeleton}, and through them \ref{thm2-1},
    \ref{thm3}, \ref{thm25}, \ref{thm27} and \ref{thm31}, all wait on it.  It is
