...
\begin{proof}
  ...
  processes.  \textbf{Not formalised}, and what is left of it is narrower than
  it was: the minorisation is \ref{aux-minorisation} and the criterion is
  \ref{aux-doeblin}, both proved, so what remains is the existence half of
  \ref{thm1-2-skeleton} and the transfer \ref{eq13}.
\end{proof}

\aux{Lemma}
\begin{lemma}[Doeblin's criterion, the uniqueness half]
  \label{aux-doeblin}
  \lean{SocialNetwork.iterateKernel, SocialNetwork.invariant_iterateKernel, SocialNetwork.eq_of_invariant_of_minorisation_on, SocialNetwork.eq_of_invariant_of_minorisation, SocialNetwork.eq_of_invariant_of_iterate_minorisation, SocialNetwork.tsum_measure_singleton, SocialNetwork.measure_singleton_of_invariant, SocialNetwork.tsum_tsub}
  \leanok
  Let $\kappa$ be a Markov kernel on a countable space, $S$ a set, $l$ a point
  and $c > 0$ with $\kappa^n(x, \{l\}) \ge c$ for every $x \in S$.  Then
  $\kappa$ has \emph{at most one} invariant probability measure carried by $S$.
\end{lemma}
\begin{proof}
  \leanok
  Write $m(x) = \min\{\mu(x), \nu(x)\}$ for two such measures and
  $T = \sum_x m(x)$, so that $\mu = \nu$ exactly when $T = 1$.  Invariance
  carries $m$ forward --- $m(y) \ge \sum_x \kappa^n(x,\{y\})\, m(x)$, since each
  of $\mu(y)$ and $\nu(y)$ is that sum against its own measure and $m$ is below
  both --- and summing over $y$ gives $T \ge T$, which says nothing.  The
  minorisation is what turns it into something: at $l$,
  \[
  ...
  and --- this is the point --- no existence theorem.  The set $S$ is there
  because the chain of the paper lives on the state space of \ref{eq2}, which is
  not all of $\mathbb{Z}^{\actors \times \opinions}$.
\end{proof}
...
\begin{theorem}[The invariant measure of the skeleton]
  ...
  \lean{SocialNetwork.existsUnique_invariantSkeleton, SocialNetwork.exists_invariantSkeleton}
  \leanok
  \uses{def3}
...
\begin{proof}
  ...
  This is the Doeblin step, and it is the keystone: \ref{thm1-2},
  \ref{thm2-1}, \ref{thm3}, \ref{thm25}, \ref{thm27} and \ref{thm31} all rest
  on it.  \textbf{Uniqueness is formalised; existence is not}, and existence is
  now all that is missing.

  \emph{Uniqueness} is done.  \ref{aux-doeblin} is the criterion and
  \ref{aux-minorisation} is the minorisation this chain satisfies, so two
  invariant probability measures carried by $\Sspace$ coincide.  No
  irreducibility, no aperiodicity, no recurrence, and --- this is what makes
  the split possible --- no existence theorem.

  \emph{Existence} is the half that does need Markov-chain theory.  [GL24] gets
  it from positive recurrence: the same minorisation bounds the return time to
  $l^o$ by $2N$ times a geometric variable, so its expectation is finite, and
  the invariant measure is then built from one excursion.  That construction is
  what \texttt{blueprint/blueprint.md} records as absent; the one
  \texttt{sorry} this node still carries is
  \texttt{SocialNetwork.exists\_invariantSkeleton}.
\end{proof}
...
The fourth group is now \emph{empty}.  Every numbered statement of the paper is
stated in Lean, and nothing here is unproved for want of doing the work; the
subsection that held that group is kept below as the record of what it cost.

\subsection*{Blocked on Mathlib}

\ref{aux-doeblin} and \ref{aux-minorisation} have changed what the first item
below means.  Doeblin's \emph{criterion} is no longer missing: it is proved
here, for a general Markov kernel on a countable space.  Nor is the
minorisation this chain satisfies, which was work in this repository rather
than a gap in Mathlib and is now done, at \ref{aux-minorisation}.  Together
they give \ref{thm1-2-skeleton} outright in one direction: the skeleton has
\emph{at most one} invariant probability measure carried by $\Sspace$.  What is
left is the \emph{existence} half --- the construction of an invariant measure
from one excursion of a positive recurrent chain --- and that is Markov-chain
theory Mathlib does not have.

\ref{thm1-1}, \ref{thm1-2} and its skeleton half \ref{thm1-2-skeleton},
\ref{thm2-1}, \ref{thm3}, \ref{thm4} and their auxiliaries.  Verified against the pinned revision
...
\begin{itemize}
  \item \textbf{Doeblin's condition $\Rightarrow$ a unique invariant measure}:
    ...
    the implication, and it is the clearest thing in this repository to offer
    upstream.)
...
processes.  The shortest path is therefore the second item alone --- the
existence of $\muskel$, everything else about it being proved: \ref{prop9} is
already proved from \ref{prop7} and~\ref{prop8}, \ref{rem5-eta} and Kac's
inequality, and \ref{thm2-1} follows from it by \ref{eq13} once $\muskel$
exists.
...
\ref{note-green}: does it follow what the paper wrote?  Fifty-two proofs
...
own, so it counts fifty-three entries.  Every one of them is named, and
...
\medskip \noindent\textbf{Follow the paper's proof} (twenty-six). \ref{prop5},
...
and \ref{rem1}, \ref{rem4} and~\ref{rem8}.  These
...
which the paper cites.  \ref{aux-nstep} identifies the $n$-step kernel with the
