...
\begin{proof}
  ...
  \ref{aux-hitting-rate} was in the way.  \ref{eq19} has since left that queue
  too, and outright: it goes through \ref{aux-restart} and the clock rather
  than through this node, and carries no \texttt{sorryAx} at all.
\end{proof}
...
\begin{remark}[Reaching $\ladder^o$ before leaving $C^o$]
  ...
  Its route is \ref{thm2-2} --- ``by the same steps as part~2 of Theorem~2'',
  as the paper says and as Remark~14 of [GL24] says for $M = 2$ --- and not
  \ref{cor11}, which this node used to be drawn from.  That edge was a
  neighbour mistaken for a dependency, and it is what filed Remark~6 behind
  Doeblin.
\end{remark}
...
as their proofs were written, and \ref{rem6} will: it is blocked on nothing
today, and rests on \ref{prop7} once written, hence on \ref{lem19}
and~\ref{lem20}.  \ref{eq19} did not join it: written out, its proof turned out
not to pass through \ref{thm2-2} at all, and it is proved.  A node is listed as
not done rather than as blocked because that is what it is: the obstruction is
recorded once, at \ref{lem19} and~\ref{lem20}, and not copied down the graph.
...
and the last of those groups is now \ref{rem6} alone, \ref{cor11}
and~\ref{eq19} having been written.
...
blocked on anything.  What survives as a
...
\ref{note-green}: does it follow what the paper wrote?  Forty-eight proofs
...
own, so it counts forty-nine entries.  Every one of them is named, and
...
\medskip \noindent\textbf{Follow the paper's proof} (twenty-four). \ref{prop5},
...
\ref{eq19} inside its own, \ref{lem13}, \ref{cor15} and~\ref{cor30},
...
predecessors of $0$, which is \ref{aux-zero-predecessors}.
...
\begin{enumerate}
  \item \textbf{What is left around part~2 of Theorem~2}.  \ref{rem6}, and
    nothing else.  It can be stated at any time: its route is \ref{thm2-2},
    ``by the same steps as part~2'', and the work is transcribing $\ladder^o$
    and $C^{-o}$ into the shape \ref{lem14} wants.  It will not be
    \emph{proved} before item~2, since it rests on \ref{prop7}; but a written
    proof turns green with no edit the moment its ancestors do, which is why
    writing it is worth doing first.

    This is what is left that is nobody's fault, now that
    \ref{aux-measurable-hitting}, \ref{prop18}, \ref{thm4} part~1, \ref{prop9},
    \ref{cor10}, \ref{aux-greedy-avoids}, \ref{aux-hitting-rate},
    \ref{thm2-2}, \ref{aux-restart}, \ref{cor11} and~\ref{eq19} are closed.
    The lemma \ref{cor11} and \ref{eq19} were waiting on --- a restart of
    $\Uproc{u}$ at the first jump, which \texttt{Trajectory.shift} does not
    give, since it restarts the skeleton and the skeleton carries no clock ---
    is \ref{aux-restart}, and it is proved outright.
