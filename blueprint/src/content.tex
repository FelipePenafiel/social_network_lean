...
\begin{remark}[How the process is built in Lean]
  ...
  obligation that is not a cylinder argument, Lemma~\ref{lem:measurable-hitting},
  has a node of its own.
\end{remark}

\begin{lemma}[Measurability of the hitting times]
  \label{lem:measurable-hitting}
...
\begin{lemma}[A biased ladder is a biased consensus state]
  \label{lem:biased-ladder-consensus}
...
\begin{proof}
  ...
  Lemma~\ref{lem:ladder-consensus}.  The $o$-column is non-negative and takes
  the value $N-1 \ge 1$, so it is not identically zero; the other columns are
  $-\gamma$ times it, hence non-positive.  Appendix~C needs it for the reason
  Section~5.3 needs the unbiased version:
  \eqref{eq:h16} is about $\ladder_\alpha^o \cup C_\alpha^{-o}$ while
  Lemma~\ref{lem:biased-hitting} bounds the hitting time of $\ladder_\alpha$.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{prop:biased-bounded, prop:biased-absorb, thm:biased-existence,
    thm:biased-concentration, thm:biased-metastability}
  \textbf{Part 1 follows the paper's proof, with its recursion run as a single fixed-point
  step.}  Proposition~\ref{prop:biased-bounded} puts the profile after $N$ expressions in
  $B_N^\alpha$ with probability at least $(NM)^{-N}$, and
  Proposition~\ref{prop:biased-absorb} takes over from there, so from every $u \in \Salpha$ a
  single actor performs every expression from the $N$-th on with probability at least a
  constant $c$, uniformly in $u$.  The paper then defines the successive failure times
  $\eta_n$, applies the strong Markov property at each and obtains
  $\mathbb{P}(\eta_{n+1} < \infty) \le (1-c)\,\mathbb{P}(\eta_n < \infty)$, whence
  $\lim_n \mathbb{P}(\eta_n < \infty) = 0$.  That limit is exactly
  $q = \sup_{v \in \Salpha} \mathbb{P}_v(\text{no actor is eventually alone})$, and the one
  inequality the induction uses --- the restart at the first failure --- gives
  $q \le (1-c)\,q$ in a single step, which forces $q = 0$ since $c > 0$.  The estimate, the
  time it is applied at and the constant are the paper's; only the bookkeeping of the
  recursion is replaced by the fixed point it converges to.
  ...
  Theorem~\ref{thm:biased-nonexplosion} is not needed for part~1: the statement is about the
  sequence of expressed pairs, and non-explosion is what makes the \emph{continuous-time}
  process well defined.

  Part 2 is Theorems~\ref{thm:biased-existence}, \ref{thm:biased-concentration}
  and~\ref{thm:biased-metastability}, stated separately and carrying their own status;
  Theorem~4 as a whole therefore rests on them.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:lemma19, lem:lemma20, lem:consensus-ladder, lem:shift}
  ...
  Lemma~\ref{lem:consensus-ladder} needs $N$ more, for a total of $(M+1)N$.
  Composing the three requires restarting the realisation at an intermediate
  time, which is Lemma~\ref{lem:shift}.  \emph{Formalised, but not sorry-free}:
  the assembly is complete in Lean and inherits \texttt{sorryAx} from
  Lemmas~\ref{lem:lemma19} and~\ref{lem:lemma20} alone.

  \textbf{Supplies a step the paper asserts, and takes a different route
  through one of them.}  The written proof carries the process to
  $\bigcup_o S^o$ \emph{at time $N+1$} --- ``so by Lemma~19 we have
  $\bigcap_{j=1}^{N+1}\xi_j^u \subset \{\Uskel{u}_{N+1} \in \bigcup_o S^o\}$''
  --- whereas Lemma~\ref{lem:lemma19} delivers that membership at the first
  repeat $\tau(u)$, which is only $\le N+1$.  Bridging the gap needs
  $\bigcup_o S^o$ to be stable under a greedy expression, and the paper neither
  states nor proves that.  It is not obvious either: a greedy expression in
  $S^o$ expresses $o$ (Lemma~\ref{lem:favouring-max}), but it resets the row of
  the actor that expresses, and that actor may be one of the very witnesses
  $a_1(u), \dots, a_{n(u)}(u)$; new witnesses have to be produced, and the
  bound on the other columns then has to be re-established, which is precisely
  the bookkeeping of Lemma~\ref{lem:lemma20} whose written invariant does not
  survive (Remark~\ref{rem:lemma20}).

  Lemma~\ref{lem:lemma20} is therefore applied here at $\tau(u)$ itself, where
  Lemma~\ref{lem:lemma19} leaves the process, and no stability of $S^o$ is
  needed.  The arithmetic is the paper's and is unchanged:
  $\tau(u) + (M-1)(n(u)+1) - 1 \le (N+1) + (M-1)N - 1 = MN$.
  ...
  stable under it (Lemma~\ref{lem:consensus-ladder}, stage~1).  So the
  formalisation keeps that bridge and drops the other.
\end{proof}
...
\begin{lemma}[asserted inside the proof of Proposition~9]
  \label{lem:greedy-avoids}
  \lean{SocialNetwork.skeleton_ne_of_greedy}
  \leanok
  \uses{prop:reach-ladder, def:steep}
  For $u \in \Sspace \setminus \steep$, the greedy run of
  Proposition~\ref{prop:reach-ladder} does not visit $u$ again: on
  $\bigcap_{j=1}^{k} \xi_j^u$ with $1 \le k \le (M+1)N$, one has
  $\Uskel{u}_k \ne u$.
\end{lemma}
\begin{proof}
  The proof of Proposition~9 reads this off Proposition~\ref{prop:reach-ladder}
  --- ``a sequence of events $\xi_j^u$ \dots leads the process to $\ladder$,
  without visiting $u$'' --- but Proposition~\ref{prop:reach-ladder} says where
  the greedy run ends and nothing about where it passes.  Once the run has
  reached $\steep$ the claim is immediate, since it stays there and
  $u \notin \steep$; what is missing is the transient.  \textbf{Not
  formalised}, and it is not a formality: greedy runs do return to matrices they
  have already visited --- from a ladder they cycle with period $N$ --- so the
  hypothesis $u \notin \steep$ is doing work.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{prop:reach-ladder, lem:eta, lem:eta-pow, prop:greedy-prob, lem:greedy-avoids}
  ...
  (Proposition~\ref{prop:reach-ladder} and
  Lemma~\ref{lem:greedy-avoids}) with the probability of
  Proposition~\ref{prop:greedy-prob}, and Lemma~\ref{lem:eta-pow} then keeps it
  in $\steep$.  Summing the geometric series
  $\sum_{m \ge 0} \eta^m = 1/(1-\eta)$ and using
  $1 - \eta \le (1 + MN) e^{-\beta(N-1)}$ gives the bound.

  \textbf{Follows the paper's proof}, and departs from it in one place.  The
  paper writes Kac's lemma as the identity
  $1/\muskel(u) = \mathbb{E}[\Rskel{u}(u)]$, which needs the chain to be
  irreducible and hence Theorem~\ref{thm:invariant-skeleton}; the proof uses
  only $\muskel(u)\,\mathbb{E}[\Rskel{u}(u)] \le 1$, and \emph{that} inequality
  ...
  kernel on a countable space
  (\texttt{SocialNetwork.kac\_tsum\_le}), by decomposing the event of visiting
  $u$ before time $m$ over the last such visit.  So this proposition does not
  wait on Doeblin's criterion, and it is stated for an arbitrary invariant
  probability measure rather than for a named $\muskel$.

  \textbf{Supplies two steps the paper asserts.}  The composition of
  Proposition~\ref{prop:reach-ladder} with Lemma~\ref{lem:eta-pow} is done on
  the realisation --- the greedy expressions and then the positive ones are one
  event of the sample space --- rather than by restarting the chain at time
  $(M+1)N$.  And the arithmetic that reaches the printed constant needs the
  regime $MN e^{-\beta/(M-1)} > 1$ to be treated separately: there $C'$ already
  exceeds $e^{\beta(N-1)}$ and $\muskel(u) \le 1$ suffices.

  \textbf{Adds the hypothesis $u \in \Sspace$}, which the paper works under
  throughout and which Proposition~\ref{prop:reach-ladder} --- called here ---
  is stated with.

  \textbf{Rests on} Lemma~\ref{lem:greedy-avoids}, and through
  Proposition~\ref{prop:reach-ladder} on Lemmas~\ref{lem:lemma19}
  and~\ref{lem:lemma20}.
\end{proof}
...
\begin{remark}[Remark~6]
  \label{rem:remark6}
  \uses{cor:hitting-zero}
  The paper's own note that the first term on the right of equation~(14) may be
  replaced by
  $\mathbb{P}(\Rhit{u}(\ladder) > t, \Rhit{u}(\ladder^o) < \Rhit{u}(C^{-o}))$,
  whence, by the same steps as part~2 of Theorem~\ref{thm:invariant}, for every
  fixed $o$ and $\delta > 0$,
  \[
  ...
  Nothing downstream uses it: it strengthens Corollary~\ref{cor:hitting-zero}
  in the direction Lemma~\ref{lem:exit-bounds} needs, and both are blocked on
  the same continuous-time analysis.
\end{remark}
...
\end{lemma}
\begin{proof}
  \uses{prop:confinement, prop:greedy-prob}
  \textbf{Not formalised, and not a numbered statement of the paper}: this
  inequality is displayed inside the proof of part~2 of
  Theorem~\ref{thm:invariant}, and Theorem~2.2 is the limit it gives at
  $t = e^{-\beta(1-\delta)/(M-1)}$.  It is stated here because
  Lemma~\ref{lem:hitting-2beta} uses the inequality and not the limit: taking
  the limit throws the rate away, so Lemma~13 cannot rest on Theorem~2.2 as
  stated.  See Section~\ref{sec:corrections}.
\end{proof}

\begin{lemma}[Equation~(19)]
  \label{lem:hitting-zero-decomp}
...
\begin{proof}
  ...
  displayed as equation~(19) inside the proof of
  Lemma~\ref{lem:hitting-2beta} and attributed there to
  Corollary~\ref{cor:hitting-zero}, which is itself stated as a limit.  Note
  that $\tau$ is declared exponential of mean $1/(MN)$, for which
  $\mathbb{P}(\tau > \beta) = e^{-MN\beta}$ and not $e^{-\beta/(MN)}$; since
  $e^{-MN\beta} \le e^{-\beta/(MN)}$ for $\beta \ge 0$, the form written here
  is the weaker of the two and Lemma~13 follows from either reading.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:hitting-zero-decomp, lem:hitting-rate, rem:bernoulli}
  Split according to whether the start is $0$.  From $0$, equation~(19) of
  Lemma~\ref{lem:hitting-zero-decomp}; from any other state the event at
  $2\beta$ is contained in the event at $\beta$.  Both are then bounded by
  Lemma~\ref{lem:hitting-rate} at $t = \beta$, and $1 - \zeta_\beta^{(M+1)N}$
  by Remark~\ref{rem:bernoulli}.  \emph{Formalised, but not sorry-free}: the
  proof is complete in Lean and inherits \texttt{sorryAx} from the two
  displays above alone.  \textbf{Supplies a step the paper asserts}: ``putting
  the inequalities above together'' is the comparison of each of the three
  terms with $e^{-\beta/((M+1)N)}$ --- using $MN \le (M+1)N$, $M-1 \le (M+1)N$
  and $e^{\beta/(M-1)} \ge 1$ --- which leaves the integer inequality
  $1 + (M+1)N \le (M+1)N^2$, true since $N \ge 3$.
\end{proof}
...
\begin{proof}
  ...
  \textbf{Follows the paper's proof of Proposition~17}, in its two halves.

  The inclusion is Proposition~\ref{prop:confinement}'s argument with $n_a$ in
  place of $\|U(a,\cdot)\|_\infty$, in its two cases: either the first $N$
  expressions come from $N$ distinct actors, and every row was reset, or some
  actor expresses at $j < k < N$ and greediness at $k$ caps the whole profile at
  $n_a = k-j-1$, which $N-k$ further expressions raise to at most $N-j-1$.
  Unlike Proposition~\ref{prop:biased-confinement}, the chain closes here:
  greediness is \emph{exact}, so the maximum at the repeat time is the expressing
  actor's own entry, with no slack of $1/(2\gamma)$ to absorb.

  The probability is the one-step bound: with $\beta \ge 0$ the maximising pairs
  carry the largest rate, so they carry at least $(NM)^{-1}$ of the total
  weight, and the iteration is the one of
  Proposition~\ref{prop:greedy-prob}.  That iteration is run once in Lean, for
  an arbitrary choice of admissible pairs at each profile, and instantiated both
  here and at Proposition~\ref{prop:biased-greedy-prob}.
\end{proof}
...
\begin{proof}
  ...
  \textbf{Not formalised, and it does not follow from Proposition~6 as the
  text asserts.}  See Remark~\ref{rem:prop22}.
\end{proof}
...
\begin{proof}
  ...
  slack of Definition~\ref{def:near-greedy}.  \textbf{Follows the paper's proof
  of Proposition~8}, which Appendix~C invokes for this statement.

  The one-step bound is the same computation: the maximising pair carries weight
  $e^{\beta y}$, every pair outside $\tilde{Y}_\gamma$ carries at most
  ...
  Remark~7 is for.  As in Proposition~\ref{prop:greedy-prob} the iteration goes
  through the finite-horizon kernels of the Ionescu--Tulcea construction rather
  than the conditioning of~(10).
\end{proof}
...
\begin{proof}
  ...
  used to be about the skeleton path measure and the discrete steps
  $k \le \lceil 2\beta \rceil$ rather than about the continuous-time hitting
  time $R^{\alpha,\beta,u}$, and it bound $C$ after $\beta$ and $u$, so the
  constant was free to depend on both.  Neither matches the display above, and
  neither can serve as \eqref{eq:h16} for
  Proposition~\ref{prop:biased-exit-exponential}, which is what this lemma
  exists for.  It now has the shape of Lemma~\ref{lem:hitting-2beta} with
  $\pressone$ replaced by $\tfrac{1}{2\gamma}$.  A formalisation-side
  correction, not a correction to the paper.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{cor:cbeta, lem:biased-exit-bounds, lem:biased-ladder-nonempty}
  The rearrangement of Corollary~\ref{cor:cbeta}, with $\pressone$ replaced by
  $\tfrac{1}{2\gamma}$: apply Lemma~\ref{lem:biased-exit-bounds}, part~1, at
  $t = c_{\alpha,\beta}$ to the ladder of
  Lemma~\ref{lem:biased-ladder-nonempty}.  \emph{Formalised, but not
  sorry-free}: complete in Lean, inheriting \texttt{sorryAx} from
  Lemma~\ref{lem:biased-exit-bounds} alone.  Its only addition to the paper is
  the witness Corollary~\ref{cor:cbeta} needed as well.
\end{proof}

\begin{lemma}[$L_\alpha^o$ is inhabited]
  \label{lem:biased-ladder-nonempty}
...
\begin{proof}
  ...
  $u(a,o) = a(1+\gamma) - \gamma a = a$ and $u(a,p) = -\gamma a$, which is
  equation~(9).  Actor $1$ has heard nothing and the $n_a$ are pairwise
  distinct, so the profile lies in $S^\alpha$.  \textbf{No counterpart in the
  paper}, exactly as for Lemma~\ref{lem:ladder-nonempty}: without it
  Corollary~\ref{cor:biased-cbeta} is vacuous.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{prop:biased-exit-exponential, thm:metastability, cor:biased-cbeta,
    lem:biased-hitting, lem:biased-exit-bounds, lem:biased-ladder-consensus}
  As Theorem~\ref{thm:metastability}, which is all Appendix~C says.
  \emph{Formalised, but neither sorry-free nor axiom-free}: the assembly is
  complete in Lean, and inherits \texttt{sorryAx} from
  Lemmas~\ref{lem:biased-hitting} and~\ref{lem:biased-exit-bounds} together
  with the second [LM22] axiom, Proposition~\ref{prop:biased-exit-exponential}.
  ...
  \begin{itemize}
    \item \eqref{eq:h15} at $s_1 = 1$,
      ...
      Lemma~\ref{lem:biased-exit-bounds} through $1 - e^{-x} \le x$;
    \item \eqref{eq:h16} at $s_2 = 2\beta$, from
      Lemma~\ref{lem:biased-hitting} and the inclusion
      $\ladder_\alpha \subseteq \ladder_\alpha^o \cup C_\alpha^{-o}$ of
      Lemma~\ref{lem:biased-ladder-consensus};
    \item \eqref{eq:h17} at $\delta = 1/(4\gamma)$, from
      Corollary~\ref{cor:biased-cbeta};
    \item \eqref{eq:h18} at $\theta = 1/(4\gamma)$, from part 2 of
      Lemma~\ref{lem:biased-exit-bounds} at $t = 2\beta$.
  \end{itemize}
...
\begin{proposition}[Proposition~12 for the biased process]
  \label{prop:biased-exit-exponential}
...
\begin{proof}
  ...
  of Theorem~31 follows exactly as the proof of Theorem~3'', and that proof runs
  through Proposition~\ref{prop:exit-exponential}, which is stated for the
  unbiased process alone.

  One axiom cannot serve both models, for the reason recorded at
  Proposition~\ref{prop:exit-exponential}: stated abstractly the assertion is
  \textbf{inconsistent}, so it has to be attached to a concrete process, and the
  unbiased axiom is attached to the unbiased one.  Everything resting on this
  one is listed separately in the CI axiom inventory.
\end{proof}
...
\begin{remark}[Proposition~22 does not follow from Proposition~6]
  \label{rem:prop22}
  \uses{def:near-greedy, prop:confinement}
...
\begin{itemize}
  ...
  \item \textbf{Recurrence for chains, and return times}: nothing.  Poincaré
    ...
    \texttt{Kac} in Mathlib is a Kac--Moody algebra --- but it turned out not to
    be needed: the inequality
    $\muskel(u)\,\mathbb{E}[\Rskel{u}(u)] \le 1$, which is all
    Proposition~\ref{prop:skeleton-concentration} uses, holds for every
    invariant probability measure and is proved outright at
    \texttt{SocialNetwork.kac\_tsum\_le}.)
...
point processes.  The shortest path is therefore Doeblin alone:
Proposition~\ref{prop:skeleton-concentration} is already proved from
Propositions~\ref{prop:reach-ladder} and~\ref{prop:greedy-prob},
Lemma~\ref{lem:eta} and Kac's inequality, and
Theorem~\ref{thm:concentration} follows from it by
Lemma~\ref{lem:transfer} once $\muskel$ exists.

\subsection*{Blocked on the paper}

Lemmas~\ref{lem:lemma19}, \ref{lem:lemma20} and~\ref{lem:greedy-avoids}, and
Proposition~\ref{prop:biased-confinement}; through them
Propositions~\ref{prop:reach-ladder}, \ref{prop:skeleton-concentration}
and~\ref{prop:biased-reach-ladder}.  See
Remarks~\ref{rem:lemma19}, \ref{rem:lemma20} and~\ref{rem:prop22}, and the node
of Lemma~\ref{lem:greedy-avoids}: in each case
the written proof does not compose, and the repair changes the argument rather
than its presentation, so it is recorded rather than guessed at in Lean.
Proposition~\ref{prop:reach-ladder} is nonetheless \emph{proved}, along a route
that does not need the step the written proof asserts --- the one departure of
that kind in this repository, and the reason is written at its node.
...
Proposition~\ref{prop:exit-exponential} and its twin
Proposition~\ref{prop:biased-exit-exponential} are Theorem~5.3 of [LM22].  They
...
--- so it has to be attached to a concrete process, and there are two processes.
They are the same citation applied twice, not two assumptions.

Lemmas~\ref{lem:hitting-rate} and~\ref{lem:hitting-zero-decomp} are the two
inequalities the paper \emph{displays inside proofs} and never states.
Lemma~\ref{lem:hitting-2beta} uses them, and it cannot use the numbered
statements they are attributed to --- Theorem~2.2 and
Corollary~\ref{cor:hitting-zero} --- because those are limits, and a limit has
thrown the rate away.  They are transcribed verbatim and carry a
\texttt{sorry}; whether either should become a numbered statement of the paper
...
Lemma~\ref{lem:measurable-hitting}, is now proved as well; the subsection after
next records what its diagnosis got wrong.  What is unproved here is blocked on
Mathlib, blocked on the paper, cited from outside it, or --- for
Remark~\ref{rem:remark6} alone --- simply not done.

\subsection*{Was here, and is now closed}

\textbf{Kac's lemma} stood in the list above as the second gap in Mathlib, and
Proposition~\ref{prop:skeleton-concentration}, Corollary~\ref{cor:zero} and
three results below them were recorded as waiting on it.  They were not.  What
the paper's proof uses is the inequality
$\muskel(u)\,\mathbb{E}[\Rskel{u}(u)] \le 1$, not the identity, and the
inequality holds for \emph{every} invariant probability measure --- the identity
is what needs irreducibility, and the counterexample is two absorbing states
with $\muskel = (\tfrac12, \tfrac12)$, where the return time is $1$ and not
$2$.  The inequality is proved at \texttt{SocialNetwork.kac\_tsum\_le}, for a
Markov kernel on a countable space, from \texttt{Kernel.Invariant} and nothing
else, by decomposing the event of visiting $u$ before time $m$ over the last
such visit.  The lesson is the same as the one below: a citation is not an
obstruction until one has checked which half of it the proof needs.

Lemma~\ref{lem:measurable-hitting} stood in this chapter as a fourth kind of
...
first clause is the error: right-continuity is \emph{a} route to the
reduction, not the only one, and the reduction holds pointwise.  The
argument that works is at Lemma~\ref{lem:measurable-hitting}; it reads the
level sets of the jump count directly, and never mentions the holding times,
so it costs nothing to transport to the biased process.
...
Remark~\ref{rem:green}: does it follow what the paper wrote?  Thirty-eight
proofs carry \texttt{\textbackslash leanok}; the audit below also covers the
bound $\tau(u) \le N+1$, which lives inside a definition rather than at a node
of its own, so it counts thirty-nine entries.

\medskip
\noindent\textbf{Follow the paper's proof} (nineteen).
Propositions~\ref{prop:low-pressure}, \ref{prop:confinement},
\ref{prop:reach-ladder}, \ref{prop:skeleton-concentration},
\ref{prop:biased-low-pressure},
\ref{prop:biased-bounded}, \ref{prop:biased-greedy-prob}
and~\ref{prop:biased-absorb}, part~1 of Theorem~\ref{thm:phase},
the gap estimate of Lemma~\ref{lem:gap},
the bound of Proposition~\ref{prop:greedy-prob},
Lemma~\ref{lem:hitting-2beta},
Corollaries~\ref{cor:cbeta} and~\ref{cor:biased-cbeta},
Theorems~\ref{thm:metastability} and~\ref{thm:biased-metastability}, and
Remarks~\ref{rem:biased-row}, \ref{rem:bernoulli} and~\ref{rem:remark8}.  These are the statements in the
...
Three more carry notes.  Proposition~\ref{prop:biased-low-pressure} is the
proof of Proposition~5 run on $n_a$ rather than on
$\|U(a,\cdot)\|_\infty$, the passage between them being
$u(a,p) \le n_a$, which equation~(6) gives but does not state.
Theorem~\ref{thm:metastability} is Section~5.3's verification of
\eqref{eq:h15}--\eqref{eq:h18}, with three steps supplied: the inclusion
$\ladder \subseteq \ladder^o \cup C^{-o}$, the supremum of~(20), and the
threshold $\beta_1$; and its Proposition~\ref{prop:exit-exponential} is an
axiom, not a theorem.  Corollary~\ref{cor:cbeta} is the paper's rearrangement,
but needs $\ladder^o \ne \emptyset$, which is
Lemma~\ref{lem:ladder-nonempty}; Corollary~\ref{cor:biased-cbeta} is that same
rearrangement with $\pressone$ replaced by $\tfrac{1}{2\gamma}$, and needs the
same witness, Lemma~\ref{lem:biased-ladder-nonempty}.
Lemma~\ref{lem:hitting-2beta} is the paper's case split, resting on two
inequalities the paper displays inside proofs rather than states ---
Lemmas~\ref{lem:hitting-rate} and~\ref{lem:hitting-zero-decomp} --- and
supplying the arithmetic of ``putting the inequalities above together''.
Theorem~\ref{thm:biased-metastability} is the same verification as
Theorem~\ref{thm:metastability} with $\pressone$ replaced by
...
\emph{second} axiom, Proposition~\ref{prop:biased-exit-exponential}, since one
cannot serve both models.
Proposition~\ref{prop:reach-ladder} is the paper's three-stage assembly with
its arithmetic unchanged, but it applies Lemma~\ref{lem:lemma20} at $\tau(u)$
rather than at $N+1$: the written route needs $\bigcup_o S^o$ to be stable
under a greedy expression, which the paper does not establish, and starting
where Lemma~\ref{lem:lemma19} actually lands removes the need.  It is the one
place where a formalised proof takes a different route through the paper's own
lemmas, and the reason is written out at the node.
Propositions~\ref{prop:biased-bounded}
and~\ref{prop:biased-greedy-prob} are the arguments of
Propositions~\ref{prop:confinement} and~\ref{prop:greedy-prob} that
Appendix~C invokes; the iteration common to the two is run once in Lean, for an
arbitrary choice of admissible pairs at each profile, since that is all either
uses.
Proposition~\ref{prop:biased-absorb} is the paper's decomposition~(21), its
bound~(22), its passage through $\ln(1+x) \ge x/(1+x)$ and its event
$E_\varepsilon^k$, in that order, with the strong law of large numbers taken from
Mathlib as the paper takes it from the literature.  Two steps are supplied: the
paper's ``without loss of generality $u(1,o) = 0$ for all $o$'', which is the
choice of an actor whose row is null and which equation~(6) always provides;
and the terms of index below $k$, which $E_\varepsilon^k$ does not constrain and
which the paper absorbs silently into its generic constant $C$.
Proposition~\ref{prop:skeleton-concentration} is the paper's own proof --- Kac's
lemma, then Proposition~\ref{prop:reach-ladder} and Lemma~\ref{lem:eta-pow} to
bound the return time below --- with one departure and two supplied steps.  The
departure is Kac: the paper states it as the identity
$1/\muskel(u) = \mathbb{E}[\Rskel{u}(u)]$, which needs irreducibility and hence
Theorem~\ref{thm:invariant-skeleton}, and the proof uses only the inequality
...
neither Doeblin's criterion nor Mathlib.  The supplied steps are the composition
of the two estimates, done on the realisation rather than by restarting the
chain, and the arithmetic of the printed constant, which needs the regime
$MNe^{-\beta/(M-1)} > 1$ treated separately.  It rests on
Lemma~\ref{lem:greedy-avoids}, the ``without visiting $u$'' its proof reads off
Proposition~\ref{prop:reach-ladder}, and adds the hypothesis $u \in \Sspace$.
Part~1 of Theorem~\ref{thm:phase} is the estimate the paper applies at the
failure times $\eta_n$, applied at the same time and with the same constant, but
run as the single fixed-point step $q \le (1-c) q$ rather than as a recursion
over $n$; the two supplied steps --- the Markov property itself, which the paper
calls strong but applies at what is for the skeleton a deterministic time, and
the measurability of the failure events --- are written out at the node.  Only
part~1 is formalised; part~2 is Theorems~\ref{thm:biased-existence},
\ref{thm:biased-concentration} and~\ref{thm:biased-metastability}, and Theorem~4
rests on them.
...
\medskip
\noindent\textbf{No counterpart in the paper} (ten).
Lemmas~\ref{lem:trust}, \ref{lem:ladder-consensus}, \ref{lem:state-along},
\ref{lem:consensus-ladder}, \ref{lem:ladder-nonempty},
\ref{lem:biased-ladder-nonempty}, \ref{lem:biased-ladder-consensus},
\ref{lem:shift} and~\ref{lem:measurable-hitting} and
Remark~\ref{rem:biased-ladder-steep},
...
\medskip
\noindent\textbf{No proof departs from the paper for convenience, and no proof
of a statement of the paper has been invented here.}  That is the result of the
audit, and the rule of Remark~\ref{rem:green} is written to keep it that way.
Where the paper's route does not close --- Lemmas~19 and~20, and
...
are transcribed as statements of their own,
Lemmas~\ref{lem:hitting-rate} and~\ref{lem:hitting-zero-decomp}, and carry
their own \texttt{sorry}.  And where the route needs a step the paper does not
have but a shorter route through the same lemmas does not ---
Proposition~\ref{prop:reach-ladder} --- the shorter route is taken and the
difference is written down, rather than the missing step being assumed.  \texttt{FOR-THE-AUTHORS.md} at the root of
the repository collects those, and the decisions they call for, in one place.
...
\begin{enumerate}
  \item Remark~\ref{rem:remark6}, the one numbered statement of the paper with
    no Lean counterpart, and Corollary~\ref{cor:zero}, whose extra exponent
    $\pressone$ needs the states from which the zero matrix can be entered to be
    described.  These two are what is left that is nobody's fault, now that
    Lemma~\ref{lem:measurable-hitting}, Proposition~\ref{prop:biased-absorb},
    Theorem~\ref{thm:phase} part~1 and
    Proposition~\ref{prop:skeleton-concentration} are closed.
  \item Lemmas~\ref{lem:lemma19} and~\ref{lem:lemma20}, with the repairs of
    Remarks~\ref{rem:lemma19} and~\ref{rem:lemma20}, and
    Lemma~\ref{lem:greedy-avoids}, the step the proof of
    Proposition~\ref{prop:skeleton-concentration} asserts.  Worth settling with
    the authors first, since the repairs change the written proofs.
    Proposition~\ref{prop:reach-ladder} then follows by assembling the first two
    with Lemma~\ref{lem:consensus-ladder}, and
    Proposition~\ref{prop:skeleton-concentration} is proved outright once the
    third is.
