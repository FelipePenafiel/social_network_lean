...
not proved --- which of four kinds of obstruction it has met.
...
\begin{remark}[What a green node claims, and what it does not]
  ...
  The rule bites hardest where the paper's proof does \emph{not} close.  There
  the statement is left resting on the step that fails, even when a different
  argument would establish it: closing it another way would record a theorem and
  lose the finding, which is the one thing this development produces that a
  reader of the paper does not already have.  \ref{thm25} is the case to look
  at --- a route that closes it is written out at
  \ref{aux-biased-minorisation} and deliberately not taken.
...
\begin{remark}[How the process is built in Lean]
  ...
  transition semigroup and the hitting times are plain definitions.  One rate is
  worth reading off at once: at the zero matrix every $\jumprate$ summand is
  $e^0 = 1$, so $\jumprate(0) = MN$ and the first holding time from $0$ is
  exponential of mean $1/(MN)$ (\texttt{SocialNetwork.totalRate\_zero}).  That
  is the $\tau$ of \ref{cor11} and of \ref{eq19}.
...
\aux{Lemma}
\begin{lemma}[The race between the clocks]
  \label{aux-clocks}
  \lean{SocialNetwork.clockSplit, SocialNetwork.measurable_clockSplit, SocialNetwork.clockSplit_preimage_pi, SocialNetwork.measurePreserving_clockSplit, SocialNetwork.pi_expMeasure_pi_Ioi, SocialNetwork.lintegral_exp_neg_expMeasure_Ioi, SocialNetwork.winsAfter, SocialNetwork.measurableSet_winsAfter, SocialNetwork.clockSplit_preimage_winsAfter, SocialNetwork.sum_subtype_ne, SocialNetwork.pi_expMeasure_winsAfter, SocialNetwork.pi_expMeasure_coord, SocialNetwork.pi_expMeasure_coord_singleton, SocialNetwork.pi_expMeasure_coord_nonpos, SocialNetwork.pi_expMeasure_ties, SocialNetwork.IsWinner, SocialNetwork.pi_expMeasure_winner_eq, SocialNetwork.pi_expMeasure_race_Ioi, SocialNetwork.firstClock, SocialNetwork.firstClock_le, SocialNetwork.firstClock_eq_iff, SocialNetwork.measurable_firstClock, SocialNetwork.isWinner_firstClock, SocialNetwork.exists_isWinner_jump, SocialNetwork.stepLaw_prod_Ioi}
  \leanok
  \uses{eq3}
  Give each pair $(a,o)$ an independent clock ringing at an exponential time of
  rate $e^{\beta u(a,o)}$.  Then for every pair $p$ and every $t \ge 0$,
  \[
    \mathbb{P}\bigl( p \text{ rings first, and after } t \bigr)
      = \frac{e^{\beta u(p)}}{\jumprate(u)} \, e^{-\jumprate(u) t} ,
  \]
  so the winner and the winning time are independent, the winner follows the
  Gibbs law of \ref{eq3} and the time is exponential of the total rate.  That is
  one step of the process, so $\mathcal{L}_\beta$ and the clocks are the same
  law.
\end{lemma}
\begin{proof}
  \leanok
  \textbf{No counterpart in the paper}, which uses the clock form at p.~16
  without stating it: its~(11) is a statement about the clocks of a
  \emph{sub-family}, and $\mathcal{L}_\beta$ as built here is the jump-hold
  form, where no sub-family has a clock of its own.

  The computation is one integral.  Splitting the product measure at the
  coordinate $p$ --- which Mathlib does along a sum of index types and along
  $\mathrm{Fin}(n+1)$, but not along one point of an arbitrary index, so it is
  done here --- the event becomes $\{(x,g) : t < x < g_j \ \forall j\}$, whose
  measure is $\int_t^\infty e^{-(\jumprate(u) - e^{\beta u(p)})x}\,
  \mathrm{d}\mathrm{Exp}(e^{\beta u(p)})(x)$, and that is
  $\frac{e^{\beta u(p)}}{\jumprate(u)} e^{-\jumprate(u)t}$.

  Two clocks ring together with probability zero, so \emph{a} winner may be
  chosen measurably without the choice mattering; the witness breaks ties by an
  order on the pairs, and the statement is proved for every measurable choice.
  Nothing here is a Poisson point process: on this sample space a homogeneous
  process of rate $\lambda$ is the sequence of partial sums of independent
  $\mathrm{Exp}(\lambda)$ variables, and what \ref{thm1-1} needs of it is that
  those sums diverge.
\end{proof}
...
\begin{proof}
  ...
  What is missing is not the Poisson theory.  On this sample space a
  homogeneous Poisson process of rate $\lambda$ is the partial sums of
  independent $\mathrm{Exp}(\lambda)$ variables, and that those diverge is the
  strong law, which Mathlib has and \ref{prop18} already uses.  What is missing
  is the sandwich itself, which the paper asserts --- ``can be constructed in
  such a way that'' --- and does not construct.  \ref{aux-clocks} is its first
  half, and the half that is new mathematics here: the clocks of the
  low-pressure pairs carry total rate at most $NMe^{\beta N}$ whatever the rest
  of the matrix does, which is what the upper bound of~(11) says, and that is
  a statement the jump-hold form cannot even express.  What remains is to carry
  the domination across the jumps, where the state and so the rates change.
\end{proof}
...
\begin{proof}
  ...
  processes.  \textbf{Not formalised.} \ref{thm1-2-skeleton} is proved, so
  $\muskel$ exists and is unique and the transfer is the only step left; but
  that transfer runs through \ref{thm1-1}, so this theorem is blocked on
  Mathlib after all.
\end{proof}

\aux{Lemma}
\begin{lemma}[Doeblin's criterion]
  \label{aux-doeblin}
  \lean{SocialNetwork.iterateKernel, SocialNetwork.invariant_iterateKernel, SocialNetwork.eq_of_invariant_of_minorisation_on, SocialNetwork.eq_of_invariant_of_minorisation, SocialNetwork.eq_of_invariant_of_iterate_minorisation, SocialNetwork.tsum_measure_singleton, SocialNetwork.measure_singleton_of_invariant, SocialNetwork.tsum_tsub, SocialNetwork.bind_apply_singleton, SocialNetwork.escape, SocialNetwork.hit_add_escape, SocialNetwork.excursion, SocialNetwork.excursionMass, SocialNetwork.excursionReturn, SocialNetwork.excursionMass_eq_add, SocialNetwork.tsum_excursionReturn_le, SocialNetwork.excursion_eq_zero_of_notMem, SocialNetwork.excursionMass_succ_le, SocialNetwork.excursionMass_le_pow, SocialNetwork.excursionFun, SocialNetwork.excursionFun_self, SocialNetwork.tsum_excursionFun_ne_top, SocialNetwork.tsum_excursionFun_mul, SocialNetwork.exists_invariant_of_weight, SocialNetwork.exists_invariant_of_minorisation, SocialNetwork.iterateKernel_absorbing, SocialNetwork.avgFun, SocialNetwork.tsum_avgFun, SocialNetwork.tsum_avgFun_mul, SocialNetwork.exists_invariant_of_invariant_iterateKernel, SocialNetwork.exists_invariant_of_iterate_minorisation, SocialNetwork.existsUnique_invariant_of_iterate_minorisation}
  \leanok
  Let $\kappa$ be a Markov kernel on a countable space, $S$ an absorbing set,
  $l \in S$ and $c > 0$ with $\kappa^n(x, \{l\}) \ge c$ for every $x \in S$.
  Then $\kappa$ has \emph{exactly one} invariant probability measure carried by
  $S$.
\end{lemma}
\begin{proof}
  \leanok
  \emph{Uniqueness.}  Write $m(x) = \min\{\mu(x), \nu(x)\}$ for two such
  measures and $T = \sum_x m(x)$, so that $\mu = \nu$ exactly when $T = 1$.
  Invariance carries $m$ forward --- $m(y) \ge \sum_x \kappa^n(x,\{y\})\, m(x)$,
  since each of $\mu(y)$ and $\nu(y)$ is that sum against its own measure and
  $m$ is below both --- and summing over $y$ gives $T \ge T$, which says
  nothing.  The minorisation is what turns it into something: at $l$,
  \[
    \mu(l) = \sum_x \kappa^n(x,\{l\})\, m(x)
      + \sum_x \kappa^n(x,\{l\})\, (\mu(x) - m(x))
      \;\ge\; \sum_x \kappa^n(x,\{l\})\, m(x) + c\,(1 - T),
  \]
  and the same for $\nu$, so the term at $l$ carries a surplus of $c(1-T)$.
  Summing over $y$ now gives $T \ge T + c(1-T)$, whence $T = 1$.

  \emph{Existence} is the excursion measure.  Write
  $Q^m(y) = \mathbb{P}_l(\Uskel{}_m = y,\ \Uskel{}_j \ne l \text{ for }
  1 \le j \le m)$ for the mass the chain started at $l$ still carries at time
  $m$ without having come back, and $\nu = \sum_m Q^m$.  Three facts finish it.
  First $\nu(l) = 1$, only $m = 0$ contributing there.  Second
  $\sum_y \nu(y) = \sum_m \mathbb{P}_l(R_l > m) < \infty$, because each step
  costs a factor $1 - c$: the minorisation says the chain returns with
  probability at least $c$ wherever it is, so $\mathbb{P}_l(R_l > m) \le
  (1-c)^m$.  Third $\nu \kappa \le \nu$, with equality off $l$ --- there the
  recursion defining $Q^{m+1}$ \emph{is} one step of $\kappa$ --- and $\le$ at
  $l$, where what comes back is the total of the returns, which telescopes
  against the masses and is therefore at most one.  A measure with $\nu\kappa
  \le \nu$, the same finite total mass on both sides, is invariant, which is
  the step that ends the uniqueness proof as well.  Normalising gives the
  measure, and it is carried by $S$ because the excursion starts in $S$ and $S$
  is absorbing.

  One reduction makes this work at one step rather than $n$: an invariant
  measure of $\kappa^n$ averages into one of $\kappa$, since
  $\sum_{i<n} \mu\kappa^{i}$ telescopes and its two ends agree.  So the
  excursion is run for $\kappa^n$, where the minorisation is a one-step bound.

  \textbf{No counterpart in the paper}, which cites Doeblin's criterion.  The
  argument is written for a general Markov kernel on a countable space because
  it is exactly as easy there, and it consumes nothing but
  \texttt{Kernel.Invariant}: no irreducibility, no aperiodicity, no recurrence
  theory and no compactness --- on a countable space the excursion measure is a
  sum in $[0,\infty]$ and nothing has to converge in a topology.  The set $S$ is
  there because the chain of the paper lives on the state space of \ref{eq2},
  which is not all of $\mathbb{Z}^{\actors \times \opinions}$.
\end{proof}

\aux{Lemma}
\begin{lemma}[The $n$-step kernel is the law of the skeleton]
  \label{aux-nstep}
  \lean{SocialNetwork.lintegral_pathMeasure_skeleton, SocialNetwork.iterateKernel_skeletonKernel, SocialNetwork.iterateKernel_succ', SocialNetwork.iterateKernel_add}
  \leanok
  \uses{def3}
  Writing $\kappa$ for the transition kernel of \ref{def3},
  $\kappa^n(v, A) = \mathbb{P}(\Uskel{v}_n \in A)$ for every $n$, every $v$ and
  every $A$.
\end{lemma}
\begin{proof}
  \leanok
  \textbf{No counterpart in the paper}, which does not distinguish the two.
  Here the distinction has to be bridged: \ref{prop6} and \ref{prop8} are
  statements about realisations, and \ref{aux-doeblin} is stated for iterates
  of a kernel.  The induction peels off the first expression, whose law is
  given by \ref{def3}, and applies the Markov property at time~1 to what
  follows; summing over that first pair is exactly the composition of kernels.
\end{proof}

\aux{Lemma}
\begin{lemma}[The minorisation of the skeleton chain]
  \label{aux-minorisation}
  \lean{SocialNetwork.descendActor, SocialNetwork.descendJump, SocialNetwork.descendState, SocialNetwork.skeleton_descendJump, SocialNetwork.skeleton_descendJump_eq_ladderOf, SocialNetwork.stepFloor, SocialNetwork.stepFloor_pos, SocialNetwork.stepFloor_le_jumpPMF, SocialNetwork.abs_express_le, SocialNetwork.abs_skeleton_le, SocialNetwork.boundedSet, SocialNetwork.greedyBound, SocialNetwork.stepFloor_pow_le_pathMeasure_descend, SocialNetwork.stepFloor_pow_le_iterateKernel, SocialNetwork.zeta_pow_le_iterateKernel_boundedSet, SocialNetwork.minorisation_iterateKernel, SocialNetwork.eq_of_invariant_skeletonKernel}
  \leanok
  \uses{def1, def3}
  There is $c > 0$, depending only on $N$, $M$ and $\beta$, with
  \[
    \kappa^{2N}\bigl(v, \{l^o\}\bigr) \;\ge\; c
    \qquad \text{for every } v \in \Sspace \text{ and every } o \in \opinions,
  \]
  where $l^o$ is the canonical ladder of \ref{def1}; one may take
  $c = \zeta_\beta^N \bigl(e^{-2\beta R/(M-1)}/MN\bigr)^N$ with
  $R = MN(M-1) + N(M-1)$.  Consequently, by \ref{aux-doeblin}, the skeleton has
  \emph{at most one} invariant probability measure carried by $\Sspace$.
\end{lemma}
\begin{proof}
  \leanok
  \uses{prop6, prop8, aux-nstep, aux-doeblin}
  \textbf{Follows the paper's argument at p.~17}, in its two halves.

  The first $N$ expressions are greedy.  \ref{prop6} then confines the matrix
  to $(-MN(M-1), N(M-1))$ entrywise, and \ref{prop8} gives that run
  probability at least $\zeta_\beta^N$, uniformly in the starting matrix.  This
  is the only half that uses $v \in \Sspace$.

  The last $N$ are the paper's~(12): the actors $N-1, N-2, \dots, 0$ express
  $o$ in that order.  Each row is reset when its actor expresses and then rises
  by $M-1$ once per later expression, so the actor expressing $j$-th from the
  end finishes at exactly the $j$-th rung --- \emph{whatever} the matrix was to
  begin with.  That is what lets the target be a single point, and it is the
  only place where the shape of $l^o$ is used.  Each of those $N$ expressions
  is made from a matrix still within $R$ of zero, one expression moving an
  entry by at most $M-1$, and the rates of \ref{eq3} are exponentials of
  $\beta u(a,o)/(M-1)$, so each has probability at least
  $e^{-2\beta R/(M-1)}/MN$ --- this is the step that needs the box, and it is
  why the sweep is preceded by a greedy run rather than applied outright.

  The two halves compose through \ref{aux-nstep}, which turns both into
  statements about $\kappa^N$: the matrix reached in between is integrated out,
  the second half being bounded below by the same constant everywhere on the
  box.

  Two remarks on what this does \emph{not} need.  The minorisation holds for
  every $o$, so the chain has $M$ such points and not one distinguished
  ladder; and nothing here is an irreducibility or aperiodicity statement ---
  \ref{aux-doeblin} consumes the display above and nothing else.
\end{proof}

\paper{Theorem 1.2}
\begin{theorem}[The invariant measure of the skeleton]
  \label{thm1-2-skeleton}
  \lean{SocialNetwork.existsUnique_invariantSkeleton, SocialNetwork.skeletonKernel_compl_stateSet, SocialNetwork.existsUnique_invariant_skeletonKernel}
  \leanok
  \uses{def3}
...
\begin{proof}
  \leanok
  \uses{aux-doeblin, aux-minorisation}
  This is the Doeblin step, and \ref{thm1-2}, \ref{thm2-1}, \ref{thm3},
  \ref{thm25}, \ref{thm27} and \ref{thm31} all rest on it.

  \ref{aux-minorisation} is the minorisation this chain satisfies and
  \ref{aux-doeblin} is the criterion, which delivers existence and uniqueness
  together: $2N$ expressions reach the canonical ladder $l^o$ with probability
  at least $c$ from every state, and that single bound is everything the
  criterion consumes.

  Two things it does \emph{not} need are worth recording.  The first is
  Markov-chain theory: nothing beyond invariance itself enters, the existence
  half being an excursion measure, which on a countable state space is a sum in
  $[0,\infty]$ rather than a limit.  And not the paper's
  $N \ge 3$: the descending sweep reaches $l^o$ for every $N \ge 1$, so the
  Lean statement drops that hypothesis.
\end{proof}
...
\begin{proof}
  \uses{thm1-1, def3}
  \textbf{Not formalised, and blocked on Mathlib} --- through \ref{thm1-1}, not
  through \ref{thm1-2-skeleton}, which is proved.  The written argument at
  p.~18 opens:

  \begin{quote}
    For a non-explosive process, a probability measure is invariant for
    $(\Uproc{u}_t)_t$ if and only if its product with the jump rate is
    invariant for the skeleton chain.  \emph{Non-explosivity holds by Part~1},
    and since every $u \in \Sspace$ carries an actor with null social pressure,
    the jump rate obeys $\jumprate(u) \ge M$ for all $u \in \Sspace$.
  \end{quote}

  ``Part~1'' is \ref{thm1-1}, which needs Poisson point processes.  The appeal
  is not rhetorical: $P_t$ is the law of $U_t$, and $U_t$ is read off the
  jump-hold representation through the jump counter, which returns a junk value
  on the explosion event.  Until that event is known to be null the hypothesis
  of invariance is not invariance for the semigroup the argument is about.

  Two of the remaining steps are this repository's to do and neither is done:
  $\jumprate(u) \ge M$ on $\Sspace$, which the null row of \ref{eq2} gives, and
  the finiteness $\sum_u \muskel(u)/\jumprate(u) \le 1/M$ that makes the
  right-hand side a probability measure.

  The step that carries the content is neither: the equivalence quoted above is
  \textbf{cited, not proved}, in the paper.  It is a standard fact about jump
  processes, and Mathlib has no form of it.  Whether it should be a third
  external citation, like \ref{prop12}, or an auxiliary node proved here, is a
  question for the authors and is recorded in \texttt{FOR-THE-AUTHORS.md}.
\end{proof}
...
\begin{theorem}[Concentration of $\muinv$]
  \label{thm2-1}
  \lean{SocialNetwork.measure_ladderSet_ge}
  \leanok
  \uses{def1, thm1-2}
  There is $C > 0$ such that $\muinv(\ladder) \ge 1 - C e^{-\beta/(M-1)}$ for
  every $\beta \ge 0$.
\end{theorem}
\begin{proof}
  \uses{prop9, eq13}
  From \ref{prop9} and the transfer of \ref{eq13}. \textbf{Not formalised}: the
  statement is about $\muinv$, and there is no $\muinv$ to be concentrated
  until \ref{thm1-2-skeleton} supplies one.
\end{proof}

\paper{Theorem 2.2}
\begin{theorem}[Reaching a ladder in time $e^{-\beta/(M-1)}$]
  \label{thm2-2}
  \lean{SocialNetwork.tendsto_hittingTime_ladderSet}
  \leanok
  \uses{def1}
  For every fixed $\delta > 0$,
  \[
    \sup_{u \in \Sspace \setminus \{0\}}
      \mathbb{P}\bigl(\Rhit{u}(\ladder) > e^{-\frac{\beta}{M-1}(1-\delta)}\bigr)
      \longrightarrow 0, \qquad \beta \to +\infty .
  \]
...
\begin{proof}
  \leanok
  \uses{aux-hitting-rate, rem4}
  The limit \ref{aux-hitting-rate} gives at $t = e^{-\frac{\beta}{M-1}(1-\delta)}$:
  there $e^{\beta/(M-1)} t = e^{\frac{\beta\delta}{M-1}} \to \infty$, so the
  second term vanishes, and the first is $1 - \zeta_\beta^{(M+1)N} \le
  (M+1)MN^2 e^{-\beta/(M-1)}$ by \ref{rem4}.  The supremum is bounded by that
  one $u$-free quantity and squeezed.  \emph{Formalised, but not sorry-free}:
  the proof is complete in Lean and inherits \texttt{sorryAx} from
  \ref{aux-hitting-rate} and from nothing else.

  \textbf{This half does not wait on Doeblin}, and until this node was split
  from \ref{thm2-1} the repository said it did.  Part~2 never mentions
  $\muinv$: it is a statement about the process started at a fixed $u$, and its
  proof is \ref{prop7} and \ref{prop8} by way of \ref{aux-hitting-rate}.  The
  $M = 2$ case is proved that way, in three lines and with no appeal to an
  invariant measure, at p.~19 of [GL24].  The split moves part~2 --- and with
  it \ref{cor11}, \ref{aux-hitting-rate}, \ref{eq19} and \ref{rem6} --- out of
  the queue behind \ref{thm1-2-skeleton} and into the one behind \ref{prop7},
  which is to say behind \ref{lem19} and~\ref{lem20}.  Writing this proof is
  what showed the split was not only bookkeeping: nothing but
  \ref{aux-hitting-rate} was in the way.  \ref{eq19} and~\ref{rem6} have since left that queue
  too, and outright: neither goes through this node in the end --- \ref{eq19}
  through \ref{aux-restart} and the clock, \ref{rem6} through the last stage of
  \ref{prop7} alone --- and neither carries \texttt{sorryAx}.
\end{proof}
...
\aux{Lemma}
\begin{lemma}[Restarting the process at the first jump]
  \label{aux-restart}
  \lean{SocialNetwork.shiftStepPath, SocialNetwork.ctsPartialTraj_succ_apply, SocialNetwork.map_ctsPartialTraj_shiftStepHistory, SocialNetwork.map_ctsTraj_shiftStepPath, SocialNetwork.ctsPathMeasure_restart, SocialNetwork.ctsPathMeasure_restart_le, SocialNetwork.exists_process_eq_shift, SocialNetwork.hittingTimeCts_le_shift}
  \leanok
  \uses{def3, aux-construction}
  Write $\sigma$ for the map that drops the first step of a realisation, and
  $\mathcal{L}_\beta(u)$ for the law of one step at $u$: the Gibbs law of
  \ref{def3} for the expressed pair, and an exponential of rate
  $\jumprate(u)$ for the holding time, independently of it.
  \begin{enumerate}
    \item \emph{The Markov property at the first jump.}  For measurable
      $B \subseteq \actors \times \opinions \times \mathbb{R}$ and measurable
      $E$,
      \[
        \mathbb{P}^{\beta,u}\bigl(\omega_0 \in B,\ \sigma\omega \in E\bigr)
          = \int_B \mathbb{P}^{\beta,\, \express{a}{o} u}(E)\;
            \mathcal{L}_\beta(u)\bigl(\mathrm{d}(a,o,s)\bigr).
      \]
    \item \emph{The hitting time after the first jump.}  If every holding time
      of $\omega$ is positive, then
      \[
        \Rhit{u}(\theta)(\omega) \;\le\; T_1(\omega)
          + \Rhit{v}(\theta)(\sigma\omega),
        \qquad v = \express{a_1}{o_1} u ,
      \]
      $v$ being the matrix the first expression reaches.
  \end{enumerate}
\end{lemma}
\begin{proof}
  \leanok
  \uses{aux-construction, aux-holding-times}
  Part~1 is Ionescu--Tulcea read backwards.  One step of the driving kernel
  keeps the history it started from and appends a step drawn from
  $\mathcal{L}_\beta$ at the matrix that history reaches; dropping the first
  entry of a history shifts that reading by one, so the shifted family of
  kernels is exactly the family that drives the process started at $v$.  An
  induction on the length of the history gives the identity for the
  finite-dimensional laws; the two laws on whole realisations are then equal
  because they agree on the cylinders, which form a $\pi$-system generating the
  product $\sigma$-algebra.  Integrating over the first step gives the display.

  Part~2 is pointwise, and splits on the jump counter.  Below the explosion
  time the counter of $\omega$ at $T_1 + s$ is one more than the counter of
  $\sigma\omega$ at $s$, and the two processes agree there.  On the explosion
  event both counters return the junk value of an unbounded supremum; the
  matrix at stake is then $v$ itself, which $\omega$ shows at $T_1$, and
  positivity of the holding times is what forces the counter of $\omega$ at
  $T_1$ to be exactly $1$.  \ref{thm1-1} would remove the second case
  altogether, and is not available; the hypothesis of positivity is discharged,
  where this lemma is used, by part~3 of \ref{aux-holding-times}.

  \textbf{No counterpart in the paper}, which restarts the process at $\tau$
  without comment.  It is the one thing \ref{cor11} and \ref{eq19} were waiting
  on.
\end{proof}
...
\begin{corollary}[Hitting a ladder from the zero matrix]
  \label{cor11}
  \lean{SocialNetwork.tendsto_hittingTime_ladderSet_zero, SocialNetwork.probHittingGTAfterFirstJump}
  \leanok
  \uses{def1}
  For every fixed $\delta > 0$,
  $\mathbb{P}\bigl(\Rhit{0}(\ladder) > \tau + e^{-\frac{\beta}{M-1}(1-\delta)}\bigr) \to 0$
  as $\beta \to +\infty$, where $\tau$ is exponential of mean $1/(MN)$,
  independent of the process that follows it.
\end{corollary}
\begin{proof}
  \leanok
  \uses{thm2-2, aux-restart, aux-holding-times}
  Condition on the first expression: it comes after an exponential time $\tau$
  of mean $1/(MN)$, since every rate at $0$ equals $1$, and lands on a state
  that is not $0$, to which \ref{thm2-2} applies.  In Lean the conditioning is
  part~1 of \ref{aux-restart} and the comparison of the two hitting times is
  part~2, whose hypothesis --- every holding time positive --- is part~3 of
  \ref{aux-holding-times}.  \emph{Formalised, but not sorry-free}: the proof is
  complete in Lean and inherits \texttt{sorryAx} from \ref{thm2-2}, and so from
  \ref{prop7}, \ref{lem19} and~\ref{lem20}, and from nothing else.  It never
  waited on Doeblin, since \ref{thm2-2} does not.

  \textbf{Which $\tau$.}  It is the process's own first jump time $T_1$, and
  nothing else will do.  The paper says ``exponential of mean $1/(MN)$,
  independent from $(\Uproc{u}_t)_t$'', and the superscript is the point: the
  independence asserted is from the process \emph{after} the jump, $u$ being
  the state it lands on.  That is what \ref{eq19} decomposes --- either the
  first jump takes longer than $\beta$, or it does not and the rest of the
  journey, which starts somewhere other than $0$, takes longer than $\beta$.

  Read the other way --- $\tau$ an independent copy, independent of
  $\Rhit{0}(\ladder)$ as well --- the corollary is \textbf{false}.  Since
  $0 \notin \ladder$ and $\Rhit{0}(\ladder) = T_1 + O(e^{-\beta/(M-1)})$, the
  left-hand side tends to $\mathbb{P}(T_1 > \tau)$ with $T_1$ and $\tau$
  independent and both exponential of rate $MN$, which is $\tfrac12$.

  \textbf{The Lean statement was wrong, and is now restated.}  It rendered the
  independence as a supremum over $s \ge 0$ of $e^{-MNs}\,
  \mathbb{P}(\Rhit{0}(\ladder) > s + e^{-\frac{\beta}{M-1}(1-\delta)})$, whose
  $s = 0$ term is $\mathbb{P}(\Rhit{0}(\ladder) >
  e^{-\frac{\beta}{M-1}(1-\delta)})$ with weight $1$ --- at least
  $e^{-MN e^{-\frac{\beta}{M-1}(1-\delta)}}$, which tends to $1$ and not to
  $0$.  It now reads
  $\mathbb{P}(\Rhit{0}(\ladder) > T_1 + e^{-\frac{\beta}{M-1}(1-\delta)})$,
  which is the corollary, with $T_1$ exponential of mean $1/(MN)$ by
  \ref{aux-construction}.  \textbf{A formalisation-side error, not an error of
  the paper}; \texttt{FOR-THE-AUTHORS.md} §2.11.  The corollary is proved under
  this reading, and only under it.
\end{proof}

\paper{Remark 6}
\begin{remark}[Reaching $\ladder^o$ before leaving $C^o$]
  \label{rem6}
  \lean{SocialNetwork.probHittingLadderFirst, SocialNetwork.tendsto_probHittingLadderFirst, SocialNetwork.le_probHittingLadderFirst, SocialNetwork.IsConsensus.notMem_consensusSetOther, SocialNetwork.jumpTime_lt_hittingTimeCts, SocialNetwork.expMeasure_Ici_of_nonneg, SocialNetwork.expMeasure_singleton, SocialNetwork.ctsPathMeasure_greedy_holdingTime_ge}
  \leanok
  \uses{def1, def2}
  ...
  \Rhit{u}(C^{-o}))$, whence, by the same steps as \ref{thm2-2}, for
...
\begin{proof}
  \leanok
  \uses{aux-consensus-ladder, aux-holding-times, prop8, rem4}
  ``By following the same steps of the proof of part~2 of Theorem~2'', as the
  paper says and as Remark~14 of [GL24] says for $M = 2$: the same two
  estimates, at the horizon $N$ instead of $(M+1)N$.  Along
  $\bigcap_{j=1}^{N} \xi_j^u$ from $u \in C^o$, \ref{aux-consensus-ladder} puts
  $\Uskel{u}_N$ in $\ladder^o$, so $\Rhit{u}(\ladder^o) \le T_N$; and every
  state passed through is in $C^o$, which is disjoint from $C^{-o}$, so
  $\Rhit{u}(C^{-o}) \ge T_{N+1} > T_N$.  The run therefore fails only if
  $T_N$ reaches $t$, and the two bounds of \ref{aux-holding-times} ---
  \ref{prop8} for the greedy event, the exponential race for the clock ---
  apply verbatim.  Taking $t = e^{-\frac{\beta}{M-1}(1-\delta)}$ and
  \ref{rem4} for the first term gives the limit.

  \textbf{The horizon has to be $N$}, not $(M+1)N$.  Continuing a greedy run
  past a ladder expresses the same opinion again and leaves $\ladder^o$, so the
  bound of part~2 --- which only claims $\ladder$ --- cannot be quoted as it
  stands.  What Remark~6 needs is the \emph{last} stage of \ref{prop7} on its
  own.

  \textbf{Proved outright}, and this node claimed the opposite until it was
  written.  The claim was that Remark~6 would rest on \ref{prop7}, hence on
  \ref{lem19} and~\ref{lem20}, because its route is \ref{thm2-2} and
  \ref{thm2-2} rests on them.  It does not: the route passes through the last
  stage of \ref{prop7} only, and that stage --- \ref{aux-consensus-ladder} ---
  is proved outright.  Starting inside $C^o$ is exactly what skips
  \ref{lem19} and~\ref{lem20}, whose work is to \emph{get} to $C^o$; neither
  \ref{thm2-2} nor \ref{cor11} is a dependency, though both are neighbours.
...
\aux{Lemma}
\begin{lemma}[The clock of a greedy run]
  \label{aux-holding-times}
  \lean{SocialNetwork.exp_le_totalRate, SocialNetwork.state_ne_zero_of_greedy, SocialNetwork.zeta_pow_le_ctsPathMeasure_greedyEvents, SocialNetwork.ctsPathMeasure_greedy_holdingTime_gt, SocialNetwork.ctsPathMeasure_holdingTime_nonpos, SocialNetwork.ctsPathMeasure_exists_holdingTime_nonpos, SocialNetwork.hittingTimeCts_le_jumpTime, SocialNetwork.expMeasure_Ioi_of_nonneg}
  \leanok
  \uses{def3, aux-construction, prop8, aux-greedy}
  Let $u \in \Sspace$, $u \ne 0$.  Along the greedy event:
  \begin{enumerate}
    \item no matrix visited is $0$, and the total rate out of a matrix of
      $\Sspace \setminus \{0\}$ is at least $e^{\beta/(M-1)}$;
    \item each holding time is therefore dominated by an exponential of that
      rate: $\mathbb{P}(\bigcap_{j \le n} \xi_j^u \cap \{E_n > s\}) \le
      e^{-e^{\beta/(M-1)} s}$ for $s \ge 0$;
    \item every holding time is almost surely positive, so $T_k$ is the time of
      the $k$-th expression and $\Rhit{u}(\theta) \le T_k$ whenever
      $\Uskel{u}_k \in \theta$;
    \item \ref{prop8} holds on this sample space as it does on the skeleton's.
  \end{enumerate}
\end{lemma}
\begin{proof}
  \leanok
  \uses{prop5, prop8}
  Part 1 is where $\Sspace$ earns its keep.  The zero row that \eqref{eq2}
  provides must, if one expression lands on $0$, be the row of the actor that
  expressed --- every other row would have to carry $M-1$ there --- so that
  actor expressed a $0$ entry, while some other actor carries a $1$.  No greedy
  expression allows that.  And a matrix of $\Sspace$ other than $0$ has an entry
  at least $1$, because a row summing to $0$ with a non-zero entry has a
  positive one; the single term $e^{\beta \cdot 1/(M-1)}$ of $\jumprate$ then
  bounds the sum below.

  Parts 2, 3 and 4 are the same induction along the finite-horizon kernels of
  \ref{aux-construction} that \ref{prop8} uses on the skeleton, and they need
  nothing of it but its two marginals: the expressed pair follows the Gibbs law
  of \eqref{eq3}, which gives 4, and the holding time is exponential of rate
  $\jumprate$, which
  gives 2 with $\mathbb{P}(E > s) = e^{-q s}$ and 3 with
  $\mathbb{P}(E \le 0) = 0$.  That $T_k$ is then the $k$-th expression time is
  the strict monotonicity of $k \mapsto T_k$.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{prop7, rem4, aux-holding-times}
  \textbf{Not a numbered statement of the paper}: this inequality is displayed
  inside the proof of \ref{thm2-2}, which is the limit it gives at
  $t = e^{-\beta(1-\delta)/(M-1)}$.  It is stated here because \ref{lem13} uses
  the inequality and not the limit: taking the limit throws the rate away, so
  Lemma~13 cannot rest on \ref{thm2-2} as stated.  Whether it should be
  numbered is a question for the authors, and one that
  Section~\ref{sec:corrections} still puts to them.

  \textbf{Follows the proof [GL24] writes out at its p.~19}, as its equations
  (16)--(18).  Split on the greedy event.  Its complement costs
  $1 - \zeta_\beta^{(M+1)N}$, by \ref{prop8} and \ref{rem4}.  On it \ref{prop7}
  puts the process in $\ladder$ by the $(M+1)N$-th expression, so
  $\Rhit{u}(\ladder) \le T_{(M+1)N}$ --- part 3 of \ref{aux-holding-times} --- and
  $T_{(M+1)N} > t$ forces one of the $(M+1)N$ holding times to exceed
  $t/((M+1)N)$, each of which costs at most
  $\exp(-e^{\beta/(M-1)} t/((M+1)N))$ by part 2.  A union bound finishes.

  \emph{Formalised, but not sorry-free}: the proof is complete in Lean and
  inherits \texttt{sorryAx} from \ref{prop7}, and so from \ref{lem19}
  and~\ref{lem20}, and from nothing else.  It was filed as a citation from
  outside the paper until [GL24] showed it is a step of the paper's own proof;
  it is neither, now that it is proved.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{aux-restart, aux-holding-times, aux-construction}
  \textbf{Not a numbered statement of the paper}: it is displayed as
  equation~(19) inside the proof of \ref{lem13} and attributed there to
  \ref{cor11}, which is itself stated as a limit.  What it actually decomposes
  is \ref{cor11}'s own proof --- the waiting time at $0$, then \ref{thm2-2} ---
  which is how Corollary~13 of [GL24] reads at $M = 2$.

  The proof follows that decomposition, in three cases.  Either some holding
  time of the realisation fails to be positive, which is null by part~3 of
  \ref{aux-holding-times}; or the first one exceeds $\beta$, which costs
  $e^{-MN\beta}$, the total rate at $0$ being $MN$ by \ref{aux-construction};
  or it does not, and then part~2 of \ref{aux-restart} turns
  $\Rhit{0}(\ladder) > 2\beta$ into $\Rhit{v}(\ladder) > \beta$ for the state
  $v \ne 0$ that the first expression reaches, whose probability part~1 of
  \ref{aux-restart} averages against the law of that expression.

  \emph{Proved outright.}  Unlike \ref{cor11}, this inequality does not go
  through \ref{thm2-2} --- only through the restart and the clock --- so it
  carries neither a \texttt{sorry} nor \texttt{sorryAx}.  Note that $\tau$ is
...
\begin{proof}
  ...
  \textbf{Nothing in either half uses the regime}: it enters only through
  $0 < \gamma$, so the argument proves the statement for every $\gamma > 0$.
  The Lean statement nonetheless carries the paper's $\alpha < 0$, since what is
  being checked here is the paper's proposition.  The observation is not idle ---
  see \ref{aux-biased-minorisation}, where the weaker form would remove the only
  obstruction left on \ref{thm25} --- and it is put to the authors rather than
  acted on.
...
\aux{Lemma}
\begin{lemma}[The minorisation of the biased skeleton]
  \label{aux-biased-minorisation}
  \lean{SocialNetwork.Bias.biasedDescendState, SocialNetwork.Bias.stateAfter_descendJump, SocialNetwork.Bias.stateAfter_descendJump_eq, SocialNetwork.Bias.biasedStepFloor, SocialNetwork.Bias.biasedStepFloor_pos, SocialNetwork.Bias.biasedStepFloor_le_biasedJumpPMF, SocialNetwork.Bias.abs_pressure_express_le, SocialNetwork.Bias.abs_pressure_stateAfter_le, SocialNetwork.Bias.biasedBox, SocialNetwork.Bias.biasedGreedyBound, SocialNetwork.Bias.biasedGreedyBound_nonneg, SocialNetwork.Bias.biasedZeta_pos, SocialNetwork.Bias.lintegral_biasedSkeletonKernel, SocialNetwork.Bias.measurable_stateAfter, SocialNetwork.Bias.lintegral_biasedPathMeasure_stateAfter, SocialNetwork.Bias.iterateKernel_biasedSkeletonKernel, SocialNetwork.Bias.biasedSkeletonKernel_compl_biasedStateSet, SocialNetwork.Bias.biasedStepFloor_pow_le_iterateKernel, SocialNetwork.Bias.biasedZeta_pow_le_iterateKernel_biasedBox, SocialNetwork.Bias.minorisation_iterateKernel}
  \leanok
  \uses{eq7, eq8}
  For $0 < \alpha < \pressone$ there is $c > 0$, depending only on $N$, $M$,
  $\alpha$ and $\beta$, with $\kappa_\alpha^{2N}(u, \{l_\alpha^o\}) \ge c$ for
  every $u \in \Salpha$ and every $o$, where $l_\alpha^o$ is the staircase
  profile of \ref{aux-biased-ladder-nonempty}.
\end{lemma}
\begin{proof}
  \leanok
  \uses{prop22, prop24, aux-nstep, aux-biased-ladder-nonempty}
  \textbf{Follows the argument of p.~17 for the unbiased model}, which is
  \ref{aux-minorisation}, with the two ingredients Appendix~C names in place of
  the two it transports.  Where p.~17 puts \ref{prop6} and \ref{prop8}
  together, here \ref{prop22} gives the box and \ref{prop24} prices the run, and
  that is exactly what Appendix~C prescribes: the first $N$ expressions are
  near-greedy, confining the profile to $(-MN, N)$ by \ref{prop22} --- so within
  $MN$ in absolute value, $N \le MN$ --- at probability at least
  $\zeta_{\alpha,\beta}^N$ by \ref{prop24}.

  The last $N$ expressions are the sweep of the paper's~(12): each actor's memory
  is reset when it expresses and then hears $o$ once per later expression, so the
  actor expressing $j$-th from the end finishes with exactly $j$ expressions of
  $o$ in memory --- whatever the profile was to begin with, which is again what
  lets the target be a single point.  Each of them is made from a profile still
  inside the box, one expression moving a pressure by at most $1 + \gamma$ by
  \ref{eq5}, and the rates of \ref{eq7} are exponentials of $\beta u(a,o)$, so
  each has probability at least $e^{-2\beta B}/MN$.

  \textbf{This rests on \ref{prop22}, which is blocked on the paper}
  (\ref{note-prop22}), and it is written out in full and left resting on it, the
  way \ref{prop7} rests on \ref{lem19} and~\ref{lem20}.  Every other step is
  proved, so the Lean assembly inherits \texttt{sorryAx} from \ref{prop22} alone.

  \textbf{A route that avoids it exists, and is not taken.} \ref{prop17} bounds
  the pressures after $N$ \emph{exactly} greedy expressions, at probability
  $(NM)^{-N}$, and its proof uses no regime hypothesis, only $\gamma > 0$; that
  bound is one-sided, but in $\gamma < \pressone$ some opinion carries at least
  $n_a/M$ of what the actor heard, so a cap on the pressures caps $n_a$ and hence
  bounds them below.  That assembles \ref{thm25} with no \texttt{sorry} at all.
  It is not Appendix~C's argument: the second step appears in no version of the
  paper, and this library formalises the paper's arguments rather than its
  statements.  It is recorded here, and in \texttt{FOR-THE-AUTHORS.md}, as
  something the authors may want --- weakening \ref{prop17} to $\gamma > 0$ costs
  nothing and would make \ref{prop22} unnecessary for \ref{thm25} --- but the
  choice is theirs, not the formalisation's.

  The two halves compose through the biased form of \ref{aux-nstep}, proved the
  same way.
\end{proof}

\paper{Theorem 25}
\begin{theorem}[The invariant measure of the biased skeleton]
  \label{thm25}
  \lean{SocialNetwork.Bias.existsUnique_biasedInvariant, SocialNetwork.Bias.IsBiasedInvariant, SocialNetwork.Bias.IsCarriedByBiasedState}
  \leanok
  \uses{eq7}
  For $\beta > 0$ and $0 < \alpha < \pressone$, the biased skeleton has a unique
  invariant probability measure $\mu_{\beta,\alpha}$ carried by $\Salpha$.
\end{theorem}
\begin{proof}
  \leanok
  \uses{aux-doeblin, aux-biased-minorisation}
  Theorem~25 of the paper bundles two claims: that the biased process does not
  explode, which is \ref{thm16} and is blocked on Mathlib, and that the biased
  skeleton has a unique invariant probability measure, which is this node.  The
  Lean statement covers the second only, and the statement above is narrowed
  to match it.

  \textbf{Written out, by the route the paper prescribes}: ``the proof of
  Theorem~25 follows exactly as the proof of Theorem~1''. \ref{aux-doeblin}
  applies to the biased skeleton verbatim, it being a Markov kernel on a
  countable space, and \ref{aux-biased-minorisation} is the minorisation it
  consumes.  Existence and uniqueness come together, as there.

  \textbf{Not proved}: that minorisation rests on \ref{prop22}, whose written
  proof does not close (\ref{note-prop22}), so this node inherits
  \texttt{sorryAx} from it and from nothing else.  A repair of \ref{prop22} ---
  or the weakening of \ref{prop17} discussed at \ref{aux-biased-minorisation}
  --- closes it immediately; both are the authors' to decide.

  \textbf{A warning to anyone tempted to close it here}: \ref{prop17} does give
  a box, and this node was once marked proved on that basis.  The mathematics
  was sound; the filing was not.  Appendix~C does not make that argument, and
  proving a statement of the paper by an argument the authors have not written
  is what \ref{note-green} forbids.
\end{proof}
...
\end{proposition}
\begin{proof}
  \uses{prop9, prop23, prop24, rem8}
  As \ref{prop9}, using \ref{rem8} and $0 \notin \Salpha$. \textbf{Not
  formalised}.  Transporting \ref{prop9} transports what it rests on: where that
  proof reaches a ladder by \ref{prop7} and prices the run by \ref{prop8}, this
  one uses \ref{prop23} and~\ref{prop24}.  That is what \ref{prop23} exists for;
  \ref{thm25} does not use it, the proof of Theorem~1 it transports naming only
...
\end{theorem}
\begin{proof}
  \uses{thm2-1, thm2-2, prop26, rem8}
  As \ref{thm2-1} and~\ref{thm2-2}, which it bundles the way Theorem~2 was
  bundled here until those two were split: the lower bound for the jump rate of
  a non-null state is
...
\begin{proof}
  \uses{lem13}
  \textbf{Not formalised.}  The Lean statement has the shape of \ref{lem13}
  with $\pressone$ replaced by $\tfrac{1}{2\gamma}$, which is what this lemma
  exists for: it has to serve as \eqref{eq:h16} for \ref{prop12-biased}.  That
  fixes two things about it.  It is about the continuous-time hitting time
  $R^{\alpha,\beta,u}$, not about the skeleton path measure and the discrete
  steps $k \le \lceil 2\beta \rceil$; and $C$ is bound before $\beta$ and $u$,
  so it may depend on neither.
\end{proof}
...
\begin{remark}[Proposition~22 does not follow from Proposition~6]
  ...
  \textbf{What this blocks is \ref{thm25}}, through
  \ref{aux-biased-minorisation}: the proof of Theorem~1 that Appendix~C
  transports takes its box from \ref{prop6}, and \ref{prop22} is that box here.
  Everything else in that minorisation is proved, so one repair closes Theorem~25
  outright.  A second route closes it without repairing anything: a Doeblin
  minorisation needs \emph{a} box, not the paper's constant, and \ref{prop17}
  gives one in any regime once its gratuitous hypothesis is dropped.  That route
  is not taken here, because the second half of it --- reading a lower bound off
  an upper one in $\gamma < \pressone$ --- is an argument the paper does not
  make; it is written out at \ref{aux-biased-minorisation} for the authors to
  take or leave.  The remark also blocks \ref{prop23}, through the horizon
  $C(\alpha,M,N)$ that a bound on the profile is what makes constant.
\end{remark}
...
Every numbered statement of arXiv:2607.19651 is stated in Lean.  What is not
proved is not proved for one of three reasons, and they are not
interchangeable:

\begin{itemize}
  \item \textbf{Blocked on Mathlib} --- the paper's proof is sound, and the
    library has no theory of the object it uses;
  \item \textbf{blocked on the paper} --- the written proof does not compose,
    and the repair is new mathematics;
  \item \textbf{cited from outside the paper} --- the statement is not a result
    of this paper at all.
\end{itemize}

There is no fourth group: nothing here is unproved merely for want of doing it.
\texttt{STATUS.md} is the generated index, statement by statement; what follows
is the argument behind it.

\subsection*{Blocked on Mathlib}

\ref{thm1-1} and \ref{thm16}, \ref{lem14} and \ref{lem29}, \ref{eq13} and
\ref{thm1-2} through it.  Verified against the pinned revision
...
\begin{itemize}
  \item \textbf{Poisson point processes}: nothing.  Mathlib has the Poisson
    \emph{distribution} on $\mathbb{N}$ and the Poisson limit theorem.  This is
    what \ref{thm1-1} and \ref{thm16} need, and it is the largest of these
    items.
  \item \textbf{The invariance correspondence for a jump process}: nothing.
    That a probability measure is invariant for a non-explosive jump process
    exactly when its product with the jump rate is invariant for the skeleton
    chain is what \ref{eq13} cites, and the paper cites it too rather than
    proving it.
  \item \textbf{Quantitative convergence to $\mathrm{Exp}(1)$}:
    ...
    the Kolmogorov-type bound Appendix~B uses is absent.  With
    \textbf{total-variation distance between measures}, of which there is
    nothing usable, this is what \ref{lem14} and \ref{lem29} need.
  \item \textbf{Doeblin's condition $\Rightarrow$ a unique invariant measure}:
    ...
    and \emph{no other file in Mathlib uses it}.  Irreducibility is the
    Meyn--Tweedie definition, two trivial instances and one monotonicity lemma,
    and nothing is derived from it.  This blocks nothing here --- \ref{aux-doeblin} proves the
    implication, existence \emph{and} uniqueness --- and it is the clearest
    thing in this repository to offer upstream.
  \item \textbf{Recurrence for chains, and return times}: nothing.  Poincar\'e
    recurrence exists for a measure-preserving \emph{map}, which does not
    transport to a kernel, and Kac's lemma is absent --- every \texttt{Kac} in
    Mathlib is a Kac--Moody algebra.  This blocks nothing here either: what
    \ref{prop9} uses is the inequality
    $\muskel(u)\,\mathbb{E}[\Rskel{u}(u)] \le 1$, which holds for every
    invariant probability measure and is proved at
    \texttt{SocialNetwork.kac\_tsum\_le}.  The \emph{identity} is what needs
    irreducibility, and two absorbing states with $\muskel = (\tfrac12,
    \tfrac12)$ refute it.
  \item \textbf{$N$ distinct naturals sum to at least $0 + 1 + \dots + (N-1)$}:
    no lemma, and none from which it follows in one step.
    \texttt{StrictMono.le\_apply} is stated only for endomorphisms, so the
    obvious route does not close; induction on the largest element does.
    Proved here, by that induction, together with the monotone form that
    \ref{rem5-eta} needs:
    \texttt{SocialNetwork.Bias.sum\_range\_card\_le\_sum} and
    \texttt{SocialNetwork.sum\_range\_add\_le\_sum}.
\end{itemize}

\ref{thm2-1} and~\ref{thm3} are \emph{not} on this list.  In the paper's own
architecture they live on the skeleton, and $\muskel$ is in hand: what
\ref{thm2-1} waits on is \ref{prop9}, hence \ref{prop7}, hence \ref{lem19}
and~\ref{lem20}, which are blocked on the paper.  \ref{thm2-2} and
\ref{cor11} are not on it either: a statement blocked on Doeblin is one that
needs $\muinv$ to exist, and part~2 of Theorem~2 never mentions $\muinv$.

\subsection*{Blocked on the paper}

\ref{lem19}, \ref{lem20}, \ref{aux-greedy-avoids} and \ref{prop22}; through
them \ref{prop7}, \ref{prop9}, \ref{prop23}, \ref{thm25}, \ref{prop26},
\ref{thm2-1}, \ref{thm27} and \ref{lem28}.  In each case the written proof does
not compose, and the repair changes the argument rather than its presentation,
so it is recorded --- at \ref{note-lem19}, \ref{note-lem20},
\ref{note-prop22} and the node of \ref{aux-greedy-avoids} --- rather than
guessed at in Lean.

The proofs downstream are written out in full all the same, so that each
inherits \texttt{sorryAx} from the obstruction and from nothing else:
\ref{prop7} and \ref{prop9} from \ref{lem19} and~\ref{lem20}, \ref{thm25} from
\ref{prop22}.  That is what makes an obstruction visible instead of absorbed,
and it is why \ref{thm25} is not proved by the route that would close it ---
see \ref{note-green} and \ref{aux-biased-minorisation}.

\ref{prop7}'s assembly takes a route that does not need the step the written
proof asserts.  It is the one departure of that kind here, and the reason is at
its node.

\subsection*{Cited from outside the paper}

\ref{prop12} and its twin for the biased process, \ref{prop12-biased}, are
Theorem~5.3 of [LM22].  They are declared as \texttt{axiom}s rather than left
carrying a \texttt{sorry}, because a \texttt{sorry} would put them in the
inventory of work someone could pick up, and they are not that.

Two are needed rather than one.  Stated abstractly, over an arbitrary family of
measures and an arbitrary hitting time, the statement is \emph{inconsistent}:
the zero measure with an empty ladder set satisfies the hypotheses vacuously
and falsifies the conclusion at $t = 0$.  So it has to be attached to a
concrete process, and there are two processes.  They are the same citation
applied twice, not two assumptions.

Nothing else here is a citation.  \ref{aux-hitting-rate} and~\ref{eq19} look
like two more and are not: they are displays inside the paper's own proofs,
left unnumbered, and [GL24] writes both out at its p.~19 --- the first as its
equations~(16)--(18), the second as its Corollary~13.  They are stated here as
nodes of their own because \ref{lem13} needs the inequalities and cannot use
the limits the numbered statements give.  \ref{eq19} is proved outright.

\section{What the formalised proofs check}
\label{sec:audit}

Every proof currently formalised, audited against the paper on the question of
\ref{note-green}: does it follow what the paper wrote?  Fifty-five proofs
...
own, so it counts fifty-six entries.  Every one of them is named, and
\texttt{scripts/status.py} checks that: a proof that carries
\texttt{\textbackslash leanok} and appears in none of the three groups below is
an unaudited proof, and it fails the build.

\medskip \noindent\textbf{Follow the paper's proof} (twenty-nine). \ref{prop5},
\ref{prop6}, \ref{prop7}, \ref{prop9}, \ref{cor10}, \ref{prop21}, \ref{prop17},
\ref{prop24} and~\ref{prop18}, part~1 of \ref{thm4}, the gap estimate of
\ref{aux-gap}, the bound of \ref{prop8}, \ref{thm2-2} and the display
\ref{aux-hitting-rate} inside its proof, \ref{cor11} and the display
\ref{eq19} inside its own, \ref{rem6}, \ref{lem13}, \ref{cor15} and~\ref{cor30},
\ref{thm3} and~\ref{thm31}, the minorisation \ref{aux-minorisation} of p.~17
and the theorem it settles, \ref{thm1-2-skeleton}, their biased twins
\ref{aux-biased-minorisation} and~\ref{thm25}, and \ref{rem1}, \ref{rem4}
and~\ref{rem8}.  These
...
The four around part~2 of Theorem~2 are the paper's own arguments: an
exponential race among the holding times of a greedy run for \ref{thm2-2}, and
conditioning on the first expression for \ref{cor11}.  The two displays inside
those proofs are formalised as statements of their own because \ref{lem13} needs
the rates that the limits throw away, and [GL24] writes both out at its p.~19 ---
\ref{aux-hitting-rate} as its equations~(16)--(18), \ref{eq19} as its
Corollary~13.  Three steps are supplied across them, and each is recorded at its
node: the union bound over the $(M+1)N$ holding times, which the paper
compresses; the restart of the process at its first jump, \ref{aux-restart},
which the paper performs without comment; and the almost-sure positivity of the
holding times, without which the jump counter is not the number of expressions.
\ref{cor10} is the paper's proof with one supplied step, the enumeration of the
predecessors of $0$, which is \ref{aux-zero-predecessors}. \ref{rem6} is
``the same steps'' the paper prescribes, at the horizon $N$ rather than
$(M+1)N$, with one supplied step of its own: that $C^o$ and $C^{-o}$ are
disjoint, which the paper uses without remarking on it.

\ref{thm1-2-skeleton} is the paper's own route --- a uniform minorisation, then
Doeblin --- with both steps supplied rather than cited: \ref{aux-minorisation}
for the first and \ref{aux-doeblin} for the second.  The paper's $N \ge 3$ is
not used, the sweep reaching $l^o$ for every $N \ge 1$, and the Lean statement
drops it.

\ref{aux-biased-minorisation} and \ref{thm25} are the same two nodes for the
biased model, and Appendix~C prescribes exactly that: Theorem~25 ``follows
exactly as the proof of Theorem~1''.  The two ingredients p.~17 puts together
are \ref{prop6} and \ref{prop8}; transported, they are \ref{prop22} and
\ref{prop24}, and those are what the minorisation here consumes.  \ref{prop24}
is proved and \ref{prop22} is not, so both nodes are written out in full and
inherit \texttt{sorryAx} from \ref{prop22} alone.

A route that closes \ref{thm25} outright exists and is deliberately not taken:
\ref{prop17} gives a box after an \emph{exactly} greedy run, at probability
$(NM)^{-N}$, using no regime hypothesis, and in $\gamma < \pressone$ its
one-sided bound gives a two-sided one because some opinion carries at least
$n_a/M$ of what the actor heard.  That second step is in no version of the
paper, so taking that route would establish a statement of the paper by an
argument the authors have not written --- see \ref{note-green}.  Both halves of
the observation are recorded, here and in \texttt{FOR-THE-AUTHORS.md}, as a
decision for the authors: weakening \ref{prop17} to $\gamma > 0$ is free and
makes \ref{prop22} unnecessary for Theorem~25.

\ref{aux-minorisation} is the argument at p.~17: a greedy run to bound the
matrix, then the sweep of~(12) to the canonical ladder, with the constants the
paper does not print worked out.  Three steps are supplied, and each is at the
node.  The paper says of~(12) only that it reaches a ladder; that it reaches
$l^o$ \emph{from any matrix whatever} --- which is what allows a single point
and hence Doeblin --- is proved here by an induction on the sweep.  The
quantitative form of ``the rates are bounded'', which the paper leaves as an
observation, is the explicit $e^{-2\beta R/(M-1)}/MN$ of
\texttt{SocialNetwork.stepFloor}, and the box is shown to travel with the sweep
rather than only to hold at its start.  And the composition of the two halves
goes through \ref{aux-nstep} rather than through the paper's conditioning,
Mathlib offering nothing of that shape --- the same departure as \ref{prop8},
for the same reason.

\medskip \noindent\textbf{Supply a step the paper asserts} (twelve).
\ref{aux-state-stable}, \ref{aux-consensus-pos}, \ref{rem5-stable},
\ref{lem20-opening}, \ref{lem20-closing}, \ref{rem5-eta}
and~\ref{rem5-eta-iter}, \ref{rem5} and~\ref{rem2}, \ref{aux-greedy-avoids}
and~\ref{aux-zero-predecessors}, and the bound $\tau(u) \le
...
N+1\}$''.  All twelve are true and none is deep; what they are not is checks of
...
\ref{prop9} without deriving it.  \ref{aux-greedy-avoids} is the one that took
an outside source to settle: the paper reads ``without visiting $u$'' off
\ref{prop7}, which does not give it, and it is Corollary~8 of [GL24].

\medskip \noindent\textbf{No counterpart in the paper} (fifteen).
\ref{aux-trust}, \ref{aux-shift}, \ref{aux-state-along},
\ref{aux-ladder-consensus}, \ref{aux-consensus-ladder},
\ref{aux-ladder-nonempty}, \ref{aux-biased-ladder-nonempty},
\ref{aux-biased-ladder-consensus}, \ref{aux-biased-ladder-steep},
\ref{aux-measurable-hitting}, \ref{aux-holding-times}, \ref{aux-restart},
\ref{aux-doeblin}, \ref{aux-nstep} and \ref{aux-clocks}, together with the
plumbing of the sample space, which is not made of numbered statements, and so
appears here only inside definitions and remarks.

The last five are the substantial ones, and none of them checks anything the
paper wrote.  \ref{aux-clocks} identifies one step of the process with a race
between exponential clocks, which is the form p.~16 argues in and not the form
\ref{eq3} is built in; the paper uses the identification without stating it.  \ref{aux-holding-times} says what the jump times of a greedy run
are, and \ref{aux-restart} is the Markov property at the first jump; the paper
uses both without stating either.  \ref{aux-doeblin} is Doeblin's criterion,
which the paper cites; both halves of it are proved here, the uniqueness half
by a coupling in coordinates and the existence half by the excursion measure,
and neither is in Mathlib.  \ref{aux-nstep} identifies the $n$-step kernel with the
law of the skeleton at time $n$: the paper works with one object where the
formalisation has two, the realisations that \ref{prop6} and \ref{prop8} are
stated on and the kernel iterates that \ref{aux-doeblin} consumes, and nothing
is being checked here except that they agree.
...
argument the authors have not seen; the statements downstream of it,
\ref{prop7} and \ref{thm25} among them, are written out and left resting on it,
which is what makes the obstruction visible instead of absorbing it.  Where the route closes but leaves the
...
and~\ref{eq19}; the first inherits \texttt{sorryAx} from \ref{lem19}
and~\ref{lem20}, and the second is proved.  And where the route needs
...
\begin{enumerate}
  \item \textbf{\ref{prop22}}, with a repair or the weakening of \ref{prop17}
    described at \ref{aux-biased-minorisation}.  Either closes \ref{thm25}
    outright, and nothing else in it is unproved.  The smallest item on this
    list, and the only one that is a single decision.
  \item \textbf{\ref{lem19} and~\ref{lem20}}, with the repairs of
    \ref{note-lem19} and~\ref{note-lem20}.  Worth settling with the authors
    first, since the repairs change the written proofs.  \ref{prop7} then
    follows by assembling them with \ref{aux-consensus-ladder}, and
    \ref{prop9}, \ref{cor10} and~\ref{aux-greedy-avoids} --- all three already
    written --- are proved outright, and \ref{thm2-1} with them.
  \item \textbf{The invariance correspondence for a jump process}, for
    \ref{eq13} and \ref{thm1-2} through it: that a probability measure is
    invariant for a non-explosive jump process exactly when its product with
    the jump rate is invariant for the skeleton.  The paper cites it; Mathlib
    does not have it.  It also needs \ref{thm1-1}, which is item~4.
  \item \textbf{Poisson point processes} and the superposition comparison, for
    \ref{thm1-1} and~\ref{thm16}, and \ref{eq13} through the first.  The
    largest item by far.
\end{enumerate}

Items 3 and 4 are contributions to Mathlib rather than to this repository, as
is \ref{aux-doeblin}, which is proved here and is the clearest thing to offer
upstream.  The engineering companion to this blueprint,
\texttt{blueprint/blueprint.md}, records exactly what the library does and does
not provide, with names, files and line numbers, checked against the pinned
revision.
