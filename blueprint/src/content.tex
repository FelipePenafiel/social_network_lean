...
proved, and the paragraph is kept as the record of what they cost.
...
\begin{itemize}
  \item \texttt{SocialNetwork.Bias.sum\_ge\_of\_injective}, that $N$ distinct
    ...
    because $s \subseteq \mathtt{range}\ a$.  The general form is
  ...
  \item \texttt{SocialNetwork.measurable\_process}.  The characterisation is
    \texttt{jumpCount\_eq\_iff}: for $k \ne 0$, $\mathtt{jumpCount}\ \omega\ t = k$
...
$\mathtt{expMeasure}$ charging only $[0,\infty)$, but not for every $\omega$.  On
a realisation with a negative holding time, or whose jump times accumulate from
the right at some $t$, $\mathtt{jumpCount}\ \omega\ \cdot$ is not
