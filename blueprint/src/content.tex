...
\begin{proof}
  ...
  \textbf{Not formalised}, and it is a step of the paper's own proof rather
  than a citation, which is where this node used to be filed.  [GL24] writes
  that proof out at its p.~19, as its equations (16)--(18): split on the greedy
  event, which puts the process in $\ladder$ by step $(M+1)N$ by \ref{prop7}
  and costs $1 - \zeta_\beta^{(M+1)N}$ by \ref{rem4}; on that event no state
  visited is $0$, so every holding time is dominated by an exponential of mean
  $e^{-\beta/(M-1)}$, and a union bound over the $(M+1)N$ of them gives the
  second term.  Every ingredient is in this library: the holding times are
  exponential of parameter $\jumprate$ by construction
  (\texttt{SocialNetwork.ctsPathMeasure}), and $\jumprate(v) \ge
  e^{\beta/(M-1)}$ for every $v \ne 0$ of $\Sspace$.
\end{proof}
...
\ref{thm2-2} joined this queue as soon as its proof was written, and
\ref{cor11}, \ref{aux-hitting-rate}, \ref{eq19} and~\ref{rem6} will: none of
...
and the last of those groups now has \ref{cor11}, \ref{aux-hitting-rate}
and~\ref{eq19} in it beside \ref{rem6}.
...
library, so they move to the last group, work not done.  What survives as a
...
\begin{enumerate}
  \item \textbf{The continuous-time estimates around part~2 of Theorem~2}.
    \ref{thm2-2} is written, and rests on \ref{aux-hitting-rate} alone; that
    display is next, and it is the only real work of the group, an exponential
    race among the holding times against the greedy event.  \ref{cor11} and
    \ref{eq19} both need a restart of $\Uproc{u}$ at the first jump, which this
    library does not yet have --- \texttt{Trajectory.shift} restarts the
    skeleton and nothing restarts the continuous-time process --- and that one
    lemma serves both.  \ref{rem6} can be stated at any time.

    This is what is left that is nobody's fault, now that
    \ref{aux-measurable-hitting}, \ref{prop18}, \ref{thm4} part~1, \ref{prop9},
    \ref{cor10} and~\ref{aux-greedy-avoids} are closed.  Nothing outside this
    repository is needed: the process, its holding times and its hitting times
    are all built.  None of them will be \emph{proved} before item~2, since
    each rests on \ref{prop7}; but a written proof turns green with no edit the
    moment its ancestors do, which is why writing them is worth doing first.
