...
\end{theorem}
\begin{proof}
  \uses{aux-doeblin, aux-minorisation, prop24}
  ...
  \textbf{Not formalised}, and what is missing is narrower --- and other ---
  than this blueprint said.  \ref{aux-doeblin} applies to the biased skeleton
  verbatim, it being a Markov kernel on a countable space, so all that is wanted
  is a minorisation of one of its iterates at a single profile, uniform over
  $\Salpha$.  Two of the three ingredients of \ref{aux-minorisation} transpose
  without trouble: a descending sweep to a canonical biased ladder, and a lower
  bound on one expression from a \emph{bounded} profile, the rates of \ref{eq7}
  being exponentials of bounded exponents just as those of \ref{eq3} are.

  \textbf{The bounded profile is the difficulty.}  The unbiased argument takes
  its box from \ref{prop6}; the counterpart here is \ref{prop22}, and
  \ref{prop22} is blocked on the paper (\ref{note-prop22}).  \ref{prop17} does
  give a box and is proved, but it is stated for $\gamma > \pressone$, the
  \emph{negative} bias regime, and Appendix~C's regime is $\gamma < \pressone$,
  so it does not apply.

  What rescues it is that a minorisation does not need the paper's constant $N$.
  Any bound depending on $\alpha, M, N$ alone will do --- exactly as
  \ref{note-prop22} records for Propositions~23 and~17 --- and the chain of
  \ref{prop6}, transported, does give one: $N - 1 + 1/(2\gamma)$.  Proving that
  weaker bound is an auxiliary lemma rather than a proof of \ref{prop22}, so
  this node is work in this repository and not a question for the authors.
  \ref{prop24} then supplies the probability of the greedy run, as \ref{prop8}
  does in the unbiased model.
\end{proof}
...
\begin{remark}[Proposition~22 does not follow from Proposition~6]
  ...
  $\alpha, M, N$ --- would settle it.  A Doeblin minorisation for the biased
  skeleton needs no more than that either, which is why \ref{thm25} is work in
  this repository rather than a question for the authors: see its node.
\end{remark}
...
\ref{eq13}, \ref{thm1-2} and~\ref{thm25}.  Nothing stands in the way of any of
the three; none is done.
...
\ref{thm25} is the biased twin of \ref{thm1-2-skeleton}: \ref{aux-doeblin}
applies to the biased chain verbatim, and what is missing is the biased
analogue of \ref{aux-minorisation}.  It is the one of the three that is not
routine, because the box it needs is \ref{prop22}, which does not compose, and
\ref{prop17}, which does, is stated for the other bias regime.  A bound weaker
than the paper's settles it and is work here; the node says which.
...
\ref{note-green}: does it follow what the paper wrote?  Fifty-three proofs
...
own, so it counts fifty-four entries.  Every one of them is named, and
...
\medskip \noindent\textbf{Follow the paper's proof} (twenty-seven). \ref{prop5},
...
and the theorem it settles, \ref{thm1-2-skeleton}, and \ref{rem1}, \ref{rem4}
and~\ref{rem8}.  These
