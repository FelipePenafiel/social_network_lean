...
\begin{proof}
  ...
  \textbf{The Lean statement is not this one, and is false as written.}  It
  renders ``$\tau$ exponential of mean $1/(MN)$, independent of the process''
  as a supremum over $s \ge 0$ of $e^{-MNs}\,
  \mathbb{P}(\Rhit{0}(\ladder) > s + e^{-\frac{\beta}{M-1}(1-\delta)})$.  At
  $s = 0$ the weight is $1$, and what is left standing is
  $\mathbb{P}(\Rhit{0}(\ladder) > e^{-\frac{\beta}{M-1}(1-\delta)})$ on its
  own.  From $0$ every rate equals $1$, so the first holding time is
  exponential of rate $MN$ and $\Rhit{0}(\ladder)$ is at least that time, since
  $0 \notin \ladder$; the probability is therefore at least
  $e^{-MN e^{-\frac{\beta}{M-1}(1-\delta)}}$, which tends to $1$ for every
  $\delta \in (0,1)$.  The supremum cannot tend to $0$.

  What excluding $0$ from \ref{thm2-2} costs is an \emph{average} against the
  law of $\tau$, which is an integral and not a supremum:
  \[
    \int_0^{\infty} MN e^{-MNs}\;
      \mathbb{P}\bigl(\Rhit{0}(\ladder) > s + e^{-\tfrac{\beta}{M-1}(1-\delta)}\bigr)
      \,\mathrm{d}s \;\longrightarrow\; 0 .
  \]
  That is what the corollary says and what the Lean statement has to become
  before it can be proved.  \textbf{A formalisation-side error, not an error of
  the paper}; it is recorded rather than quietly repaired because restating a
  numbered result is a choice, and \ref{eq19} --- which the paper attributes to
  this corollary --- is unaffected, being an inequality with no $\tau$ in it.
\end{proof}
...
\begin{enumerate}
  \item \textbf{The continuous-time estimates around part~2 of Theorem~2}.
    ...
    race among the holding times against the greedy event.  \ref{cor11} needs a
    first-jump decomposition of \texttt{ctsPathMeasure}, which this library does
    not yet have --- and its Lean statement has to be repaired first, for the
    reason written at its node.  \ref{eq19} follows the same decomposition, and
