...
\end{lemma}
\begin{proof}
  The proof of Proposition~9 reads this off \ref{prop7} --- ``a sequence of
  events $\xi_j^u$ \dots leads the process to $\ladder$, without visiting $u$''
  --- but \ref{prop7} says where the greedy run ends and nothing about where it
  passes.  Once the run has reached $\steep$ the claim is immediate, since it
  stays there and $u \notin \steep$; what is missing is the transient.
  \textbf{Not formalised}, and it is not a formality: greedy runs do return to
  matrices they have already visited --- from a ladder they cycle with period
  $N$ --- so the hypothesis $u \notin \steep$ is doing work.
\end{proof}
...
\begin{remark}[Reaching $\ladder^o$ before leaving $C^o$]
  ...
  \Rhit{u}(C^{-o}))$, whence, by the same steps as part~2 of \ref{thm1-2}, for
...
\begin{proof}
  ...
  inequality is displayed inside the proof of part~2 of \ref{thm1-2}, and
...
\begin{enumerate}
  \item \ref{rem6}, the one numbered statement of the paper with
    no Lean counterpart, and \ref{cor10}, whose extra exponent $\pressone$
    needs the states from which the zero matrix can be entered to be described.
    These two are what is left that is nobody's fault, now that
    \ref{aux-measurable-hitting}, \ref{prop18}, \ref{thm4} part~1 and
    \ref{prop9} are closed.
