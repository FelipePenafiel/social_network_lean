...
\begin{proof}
  ...
  \textbf{The Lean hypothesis is weaker than the statement above}: $\gamma > 0$
  rather than $\alpha < 0$.  Nothing in either half uses the regime --- it
  entered only through $0 < \gamma$ --- and the weaker form is what
  \ref{aux-biased-minorisation} needs, Appendix~C's regime being the opposite
  one.  Stating it as the paper does would have left \ref{thm25} looking blocked
  when it is not.
...
\begin{lemma}[The minorisation of the biased skeleton]
  ...
  \lean{SocialNetwork.Bias.biasedDescendState, SocialNetwork.Bias.stateAfter_descendJump, SocialNetwork.Bias.stateAfter_descendJump_eq, SocialNetwork.Bias.biasedStepFloor, SocialNetwork.Bias.biasedStepFloor_pos, SocialNetwork.Bias.biasedStepFloor_le_biasedJumpPMF, SocialNetwork.Bias.abs_pressure_express_le, SocialNetwork.Bias.abs_pressure_stateAfter_le, SocialNetwork.Bias.biasedBox, SocialNetwork.Bias.biasedGreedyBound, SocialNetwork.Bias.exists_heard_le_mul_count, SocialNetwork.Bias.heard_le_of_pressure_le, SocialNetwork.Bias.neg_le_pressure_of_pressure_le, SocialNetwork.Bias.biasedBounded_subset_biasedBox, SocialNetwork.Bias.lintegral_biasedSkeletonKernel, SocialNetwork.Bias.measurable_stateAfter, SocialNetwork.Bias.lintegral_biasedPathMeasure_stateAfter, SocialNetwork.Bias.iterateKernel_biasedSkeletonKernel, SocialNetwork.Bias.biasedSkeletonKernel_compl_biasedStateSet, SocialNetwork.Bias.biasedStepFloor_pow_le_iterateKernel, SocialNetwork.Bias.inv_pow_le_iterateKernel_biasedBox, SocialNetwork.Bias.minorisation_iterateKernel}
  \leanok
  \uses{eq7, eq8}
...
\begin{proof}
  \leanok
  \uses{prop17, aux-nstep, aux-biased-ladder-nonempty}
  \textbf{Follows the argument of p.~17 for the unbiased model}, which is
  \ref{aux-minorisation}, with one ingredient replaced.  The two halves are as
  there.  The last $N$ expressions are the sweep of the paper's~(12): each
  actor's memory is reset when it expresses and then hears $o$ once per later
  expression, so the actor expressing $j$-th from the end finishes with exactly
  $j$ expressions of $o$ in memory --- whatever the profile was to begin with,
  which is again what lets the target be a single point.  Each of them is made
  from a profile still inside a box, one expression moving a pressure by at most
  $1 + \gamma$ by \ref{eq5}, and the rates of \ref{eq7} are exponentials of
  $\beta u(a,o)$, so each has probability at least $e^{-2\beta B}/MN$.

  \textbf{The box is where this differs from \ref{aux-minorisation}}, and where
  it differs from what Appendix~C prescribes.  The unbiased argument takes its
  box from \ref{prop6} and prices the run with \ref{prop8}; the counterpart
  Appendix~C names is \ref{prop22}, which is blocked on the paper
  (\ref{note-prop22}).  It is not needed.  \ref{prop17} gives a box after $N$
  \emph{exactly} greedy expressions, at probability $(NM)^{-N}$ --- the
  maximiser being the most probable of $MN$ choices, which needs no gap
  estimate --- and its proof uses no regime hypothesis at all, only
  $\gamma > 0$.  The paper states it for $\alpha < 0$ because that is where
  Section~5.4 uses it; the Lean statement has been weakened to $\gamma > 0$ and
  serves here as well.

  One step is supplied.  \ref{prop17} bounds the pressures from above, and the
  rates want a bound on both sides.  In this regime $\gamma < \pressone$ the
  first gives the second: some opinion carries at least $n_a/M$ of what the
  actor heard, so a cap $B$ on the pressures caps $n_a$ by
  $MB/(1 - (M-1)\gamma)$, and $u(a,p) \ge -\gamma n_a$.  This is the only place
  where Appendix~C's regime is used, and it is used nowhere else in the proof.
...
\begin{proof}
  ...
  \textbf{Proved}, and by the route the paper prescribes: ``as \ref{thm1-2}''.
  \ref{aux-doeblin} applies to the biased skeleton verbatim, it being a Markov
  kernel on a countable space, and \ref{aux-biased-minorisation} is the
  minorisation it consumes.  Existence and uniqueness come together, as there.

  \textbf{One correction to record}, because this blueprint said otherwise
  twice.  It first said the biased minorisation was routine; then, correcting
  that, it said the box it needs is \ref{prop22} --- blocked on the paper ---
  and that only a constant weaker than the paper's would avoid it.  Both were
  wrong about the same thing.  \ref{prop17} gives the box, and its proof uses
  no regime hypothesis at all: the hypothesis $\alpha < 0$ in its statement is
  the regime Section~5.4 applies it in, not one the argument needs.  Weakening
  the Lean statement to $\gamma > 0$ is all it took, and \ref{prop22} is not
  used here at all.
\end{proof}
...
\begin{remark}[Proposition~22 does not follow from Proposition~6]
  ...
  $\alpha, M, N$ --- would settle it.  \ref{thm25} used to be listed as waiting
  on this; it is not, and it is proved.  A Doeblin minorisation needs a box, not
  the paper's constant, and \ref{prop17} gives one in any regime once its
  gratuitous hypothesis is dropped --- see \ref{aux-biased-minorisation}.  What
  this remark still blocks is \ref{prop22} itself, and \ref{prop23} through it.
\end{remark}
...
done.  \ref{thm25} was listed here and is now proved, by
\ref{aux-biased-minorisation}.
...
biased model, and Appendix~C prescribes exactly that: Theorem~25 ``as
Theorem~1.2''.  One ingredient is substituted and the substitution is the
interesting part.  Appendix~C's box is \ref{prop22}, which does not compose;
\ref{prop17} gives a box instead, after an \emph{exactly} greedy run rather
than a near-greedy one, at probability $(NM)^{-N}$ --- the maximiser is the
most probable of $MN$ choices, so no gap estimate is needed --- and its
argument uses no regime hypothesis, only $\gamma > 0$.  Its Lean statement has
been weakened accordingly; the paper's $\alpha < 0$ is where Section~5.4
applies it, not what it needs.  One step is supplied, and it is the only place
Appendix~C's regime is used: \ref{prop17} bounds the pressures above, the rates
want both sides, and for $\gamma < \pressone$ the one gives the other, since
some opinion carries at least $n_a/M$ of what the actor heard.
