...
\begin{proof}
  ...
  What is \emph{not} proved is the iterate $\eta^m$,
  \texttt{SocialNetwork.eta\_pow\_le\_pathMeasure\_steepLadder}: it needs the
  conditioning of Proposition~\ref{prop:greedy-prob} again, on an event that
...
\end{proposition}
\begin{proof}
  \uses{prop:reach-ladder, lem:eta, prop:greedy-prob}
  ...
  Proposition~\ref{prop:greedy-prob}, and Lemma~\ref{lem:eta} then keeps it in
  $\steep$.  \textbf{Not formalised}: Mathlib has no Kac lemma.
\end{proof}
...
Remark~\ref{rem:green}: does it follow what the paper wrote?  Twenty-one proofs
...
\medskip
\noindent\textbf{Supply a step the paper asserts} (nine).
Lemmas~\ref{lem:state-stable}, \ref{lem:consensus-pos}, \ref{lem:steep-stable},
\ref{lem:favouring-max}, \ref{lem:closing} and~\ref{lem:eta},
Remarks~\ref{rem:ladder-steep} and~\ref{rem:degenerate}, and the bound
...
$\tau(u) \in \{2, \dots, N+1\}$''.  All nine are true and none is deep; what
they are not is checks of anything the paper argued.  Lemma~\ref{lem:eta} is the
substantial one: the paper writes the two inequalities defining $\eta$ in a
single line and argues neither.
...
Six statements were listed here as neither deep nor blocked.  They are now
...
them, so none of these is a check of anything it wrote.  The last two, added
when the first four were cleared, do not: Remark~\ref{rem:remark8} is argued in
the paper and the Lean proof follows it, and Lemma~\ref{lem:eta} supplies an
...
Still here, and still routine: the iterate $\eta^m$,
\texttt{SocialNetwork.eta\_pow\_le\_pathMeasure\_steepLadder}.  It is
Proposition~\ref{prop:greedy-prob}'s induction along the Ionescu--Tulcea
kernels, run against Remark~\ref{rem:ladder-steep} and Lemma~\ref{lem:eta}
instead of the one-step greedy bound; the event moves with the state, so the
finite-horizon step needs restating, but nothing new is needed to prove it.
...
\begin{enumerate}
  ...
  \item The iterate $\eta^m$,
    \texttt{SocialNetwork.eta\_pow\_le\_pathMeasure\_steepLadder}, now that
    Lemma~\ref{lem:eta} gives the one-step bound: rerun the induction of
    Proposition~\ref{prop:greedy-prob} along the finite-horizon kernels, on the
    event $\{ \Uskel{\hat{l}}_k \in \steep \ \forall k \le m \}$.
...
Items 5, 6 and 7 are contributions to Mathlib rather than to this repository.
